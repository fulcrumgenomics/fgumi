...
impl Record {
    ...
    /// Returns the raw BAM record bytes.
    ///
    /// This returns the internal buffer containing the raw BAM binary data,
    /// which is useful for direct byte-level operations and sorting.
    #[inline]
    pub fn raw_bytes(&self) -> &[u8] {
        &self.0.buf
    }

    /// Consumes the record and returns the raw BAM bytes.
    ///
    /// This is useful when you need ownership of the underlying bytes.
    #[inline]
...
impl TryFrom<Vec<u8>> for Record {
    ...
    fn try_from(buf: Vec<u8>) -> Result<Self, Self::Error> {
        Fields::try_from(buf).map(Record)
    }
}
