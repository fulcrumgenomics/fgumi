...
use noodles_sam::{self as sam, alignment::{Record, RecordBuf}};

use self::{
    bin::write_bin, cigar::overflowing_write_cigar_op_count, cigar::write_cigar,
    cigar::write_cigar_from_slice, data::write_data, data::write_data_fast, flags::write_flags,
    mapping_quality::write_mapping_quality, name::write_name, position::write_position,
    ...
    R: Record + ?Sized,
{
    ...
    FIXED_OVERHEAD + seq_len + (seq_len + 1) / 2 + 64
}
...
#[inline]
pub fn encode_with_prealloc<R>(dst: &mut Vec<u8>, header: &sam::Header, record: &R) -> io::Result<()>
where
    R: Record + ?Sized,
{
...
#[inline]
pub fn encode_record_buf(dst: &mut Vec<u8>, header: &sam::Header, record: &RecordBuf) -> io::Result<()> {
    // Pre-allocate buffer
    let estimated_size = estimate_record_size_fast(record);
    let current_len = dst.len();
    let available = dst.capacity() - current_len;
