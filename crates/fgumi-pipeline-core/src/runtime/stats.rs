    ...
    pub fn snapshot_with_edges(
        ...
        snap.edges = edges
            ...
            .map(|e| {
                ...
                compute_edge_stats(e.producer_name, e.consumer_name, &ms, limit_bytes, wall_ns)
            })
            .collect();
        snap
    }
...
/// in ITEMS = `(mean_occupancy_fraction * limit_bytes) / mean_item_bytes`.
#[must_use]
...
fn derived_latency_ms(
    ...
    if limit == 0 || ms.pushed_items == 0 || items_per_s <= 0.0 {
        return None;
    }
    ...
    let mean_items = (f64::from(ms.mean_occupancy) * limit as f64) / mean_item_bytes;
    Some(1000.0 * mean_items / items_per_s)
}

#[must_use]
#[allow(clippy::cast_precision_loss)]
pub fn compute_edge_stats(
    producer: &'static str,
...
#[must_use]
#[allow(clippy::cast_precision_loss)]
pub fn bottleneck_verdict(snap: &StatsSnapshot) -> Vec<Finding> {
    use OccupancyClass::{Backpressured, Empty, Full, Starved};
    let mut findings = Vec::new();
    let total_cpu_ns: u64 = snap.steps.iter().map(|(_, s)| s.total_run_ns).sum();
    let class_of = |producer_match: bool, name: &str| -> Option<OccupancyClass> {
        snap.edges
            .iter()
            .find(|e| if producer_match { e.producer == name } else { e.consumer == Some(name) })
            .map(|e| e.class)
    };

    // Primary bottleneck: full input edge + empty output edge.
    for (name, s) in &snap.steps {
        let input_full = matches!(class_of(false, name), Some(Full | Backpressured));
        let output_empty = matches!(class_of(true, name), Some(Empty | Starved));
        if input_full && output_empty {
            let cpu_share =
...
impl StatsSnapshot {
    ...
    fn write_edges(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(
            f,
            "  {:<40} {:>12} {:>10} {:>14} {:>6} {:>6} {:>10}",
            "producer→consumer", "items/s", "MB/s", "class", "rej%", "empt%", "lat_ms",
        )?;
        for e in &self.edges {
...
mod tests {
    ...
    fn edge_stats_byte_bounded_latency_is_dimensionally_correct() {
        use crate::runtime::metrics::RawOccupancy;
        // 1000 items/s; mean_item_bytes = 100_000/1000 = 100; limit 2000 @ 50%
        // → mean_bytes 1000 → mean_items 10 → latency = 1000*10/1000 = 10ms.
        let ms = edge_ms(1000, 100_000, 1000, 0, 0, RawOccupancy::Healthy, 0.5);
        let e = compute_edge_stats("p", Some("c"), &ms, Some(2000), 1_000_000_000);
    ...
    fn edge_stats_count_edge_has_no_latency() {
        use crate::runtime::metrics::RawOccupancy;
        let ms = edge_ms(500, 0, 500, 0, 0, RawOccupancy::Unknown, 0.0);
        let e = compute_edge_stats("p", Some("c"), &ms, None, 1_000_000_000);
    ...
    fn occupancy_class_refined_by_rates() {
        ...
        let starved = edge_ms(10, 0, 10, 90, 0, RawOccupancy::MostlyEmpty, 0.0);
        assert_eq!(
            compute_edge_stats("p", None, &starved, None, 1_000_000_000).class,
            OccupancyClass::Starved
        );
        // MostlyEmpty + low empty-rate → Empty (idle, not starved).
        let empty = edge_ms(100, 0, 100, 0, 0, RawOccupancy::MostlyEmpty, 0.0);
        assert_eq!(
            compute_edge_stats("p", None, &empty, None, 1_000_000_000).class,
            OccupancyClass::Empty
        );
        // MostlyFull + high reject-rate → Backpressured.
        let bp = edge_ms(10, 0, 10, 0, 90, RawOccupancy::MostlyFull, 1.0);
        assert_eq!(
            compute_edge_stats("p", None, &bp, Some(1000), 1_000_000_000).class,
            OccupancyClass::Backpressured
        );
    }
    ...
    fn verdict_locates_cpu_bound_bottleneck() {
        ...
        let snap = StatsSnapshot {
            ...
            detached: vec![],
            edges: vec![
                edge_stat("Up", Some("Slow"), OccupancyClass::Full, 0.0),
                edge_stat("Slow", Some("Down"), OccupancyClass::Empty, 0.0),
            ],
        };
    ...
    fn verdict_flags_spin_and_starvation() {
        let snap = StatsSnapshot {
            ...
            workers: vec![],
            detached: vec![],
            // A starved edge: consumer frequently finds it empty.
            edges: vec![edge_stat("Producer", Some("Consumer"), OccupancyClass::Starved, 0.7)],
        };
        let v = bottleneck_verdict(&snap);
        assert!(
    ...
    fn verdict_reports_latency_bound_when_no_extremes() {
        let snap = StatsSnapshot {
            ...
            workers: vec![],
            detached: vec![],
            edges: vec![edge_stat("A", Some("B"), OccupancyClass::Healthy, 0.0)],
        };
        let v = bottleneck_verdict(&snap);
        assert_eq!(v.len(), 1);
