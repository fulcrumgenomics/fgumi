...
/// precise.
///
/// Deliberately **not** `Copy`/`Clone`: a key is a single-use token that
/// balances exactly one `prepare_wait` increment. `wait` and `cancel_wait` each
/// consume it by value, so the type system forbids handing one key to both (or
/// to either twice) — which would decrement `waiters` more than once and can
/// wrap the counter, causing a later `notify_one` to skip an armed worker until
/// its timeout.
...
impl PoolEventCount {
    ...
    /// increment.
    ///
    /// Takes the `WaitKey` **by value** to consume it: the token is single-use
    /// (see [`WaitKey`]), so taking it by reference would let one key balance
    /// the increment twice. That is why the `needless_pass_by_value` lint is
