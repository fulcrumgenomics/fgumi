...
pub struct RegisteredEdge {
    ...
    /// On an **ordered** edge, `metrics.pushed_bytes` counts the transport's
    /// `Sequenced<T>` (item + ordinal) while `popped_bytes` counts the bare `T`
    /// popped after the `ReorderStage`, so the two byte totals differ by the
    /// per-item ordinal overhead. Item counts and occupancy are unaffected; a
    /// byte-rate consumer (e.g. Little's-Law `mean_item_bytes`) should derive
    /// from `pushed_bytes / pushed_items` and treat the gap as approximate.
    pub metrics: std::sync::Arc<crate::runtime::metrics::EdgeMetrics>,
    /// `Some` for byte-bounded edges — `current_bytes / limit_bytes` gives the
    /// occupancy fraction the sampler records. `None` for count/unbounded edges
    /// (counters still apply; occupancy histogram is skipped). **Caveat:** for an
    /// ordered edge this reflects the transport queue only, not the
    /// `ReorderStage` overflow stash, so occupancy is under-reported when items
    /// pile in the reorder buffer under producer skew (a Task-4 sampler
    /// refinement could add `ReorderCapHandle` buffered bytes).
