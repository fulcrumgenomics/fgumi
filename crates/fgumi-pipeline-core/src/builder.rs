...
impl Pipeline {
    ...
    pub fn run(self, config: PipelineConfig) -> Result<(), super::signal::PipelineError> {
        ...
        use super::runtime::{
            ...
            build_chain_contexts, build_worker_storage, extract_detached_steps, is_fusible_chain,
            run_detached_step, run_fused_single_thread, run_worker_loop,
        };
        ...
        // 4d. Spawn one dedicated OS thread per `Detached` step (the sort
        // merge + writer), in chain order, BEFORE the workers — same lifecycle
        // slot as the monitor / rebalancer / sampler. Each thread drives its
        // step with a private blocking-style loop (`run_detached_step`) off the
        // work-stealing pool, so a Detached step never consumes a pool worker
        // slot (legacy's "N + 2" threading). Joined after the workers (step 6d).
        // Empty for every non-sort chain, so this is a no-op there.
        let detached_handles: Vec<thread::JoinHandle<()>> = detached_steps
            .into_iter()
            .map(|(step_idx, step)| {
                let contexts_clone = Arc::clone(&contexts);
                let signal_clone = Arc::clone(&signal_arc);
                let counter = Arc::clone(&drain_counters[step_idx.0]);
                let stats_clone = stats_arc.as_ref().map(Arc::clone);
                let name = step.name();
                thread::Builder::new()
                    .name(format!("fgumi-detached-{name}"))
                    .spawn(move || {
                        // Catch a Detached-thread panic so we can signal
                        // cancellation before unwinding — a wedged pool worker
                        // parked on this step's (now-dead) edge only exits on
                    ...
                    .expect("failed to spawn detached step thread")
            })
            .collect();
...
mod tests {
    ...
    fn instrumentation_level_predicates() {
        use InstrumentationLevel as L;
        assert!(!L::Off.is_on());
        assert!(L::Summary.is_on() && !L::Summary.timeline() && !L::Summary.deep());
        assert!(L::Timeline.is_on() && L::Timeline.samples() && L::Timeline.timeline());
        assert!(!L::Timeline.deep());
        assert!(L::Deep.deep() && L::Deep.timeline() && L::Deep.samples());
        assert_eq!(L::default(), L::Off);
        assert_eq!(PipelineConfig::default().instrumentation, L::Off);
