...
impl BackpressureState {
    ...
    pub fn sample(graph: &StageGraph) -> Self {
        Self {
            output_high: graph.q_compressed.fill_ratio() > 0.75,
...
pub struct WriteState {
    ...
    pub reorder_buf: ReorderBuffer<CompressedBatch>,
...
pub struct StageGraph {
    // Stages.
    pub group_assign: Arc<dyn PipelineStage<Input = PositionGroupBatch, Output = Vec<MiGroup>>>,
    pub consensus: Arc<
    ...
    pub compress: Arc<dyn PipelineStage<Input = Vec<u8>, Output = CompressedBatch>>,

    // Queues (ordered upstream to downstream).
    pub q_position_batch: WorkQueue<SequencedItem<PositionGroupBatch>>,
    pub q_mi_batch: WorkQueue<SequencedItem<Vec<MiGroup>>>,
    pub q_consensus: WorkQueue<SequencedItem<fgumi_consensus::caller::ConsensusOutput>>,
    pub q_filtered: WorkQueue<SequencedItem<Vec<u8>>>,
    pub q_compressed: WorkQueue<SequencedItem<CompressedBatch>>,

    // Write state (mutex-protected, highest priority).
    pub write_state: Arc<Mutex<WriteState>>,

    // Per-stage stats.
    pub stats: [Arc<StageStats>; NUM_STAGES],
...
fn try_push_held_group_assign(graph: &StageGraph, worker: &mut WorkerState) -> bool {
...
fn try_push_held_consensus(graph: &StageGraph, worker: &mut WorkerState) -> bool {
...
fn try_push_held_filter(graph: &StageGraph, worker: &mut WorkerState) -> bool {
...
fn try_push_held_compress(graph: &StageGraph, worker: &mut WorkerState) -> bool {
...
fn try_step_group_assign(graph: &StageGraph, worker: &mut WorkerState) -> StepResult {
...
fn try_step_consensus(graph: &StageGraph, worker: &mut WorkerState) -> StepResult {
...
fn try_step_filter(graph: &StageGraph, worker: &mut WorkerState) -> StepResult {
...
fn try_step_compress(graph: &StageGraph, worker: &mut WorkerState) -> StepResult {
...
fn try_step_write(graph: &StageGraph, worker: &mut WorkerState) -> StepResult {
...
// ============================================================================
// Public wrappers for try_step_* and try_push_held_* — used by builder.rs
// ============================================================================

/// Public wrapper for [`try_step_group_assign`].
pub fn try_step_group_assign_pub(graph: &StageGraph, worker: &mut WorkerState) -> StepResult {
    try_step_group_assign(graph, worker)
}

/// Public wrapper for [`try_step_consensus`].
pub fn try_step_consensus_pub(graph: &StageGraph, worker: &mut WorkerState) -> StepResult {
    try_step_consensus(graph, worker)
}

/// Public wrapper for [`try_step_filter`].
pub fn try_step_filter_pub(graph: &StageGraph, worker: &mut WorkerState) -> StepResult {
    try_step_filter(graph, worker)
}

/// Public wrapper for [`try_step_compress`].
pub fn try_step_compress_pub(graph: &StageGraph, worker: &mut WorkerState) -> StepResult {
    try_step_compress(graph, worker)
}

/// Public wrapper for [`try_step_write`].
pub fn try_step_write_pub(graph: &StageGraph, worker: &mut WorkerState) -> StepResult {
    try_step_write(graph, worker)
}

/// Public wrapper for [`try_push_held_group_assign`].
pub fn try_push_held_group_assign_pub(graph: &StageGraph, worker: &mut WorkerState) -> bool {
    try_push_held_group_assign(graph, worker)
}

/// Public wrapper for [`try_push_held_consensus`].
pub fn try_push_held_consensus_pub(graph: &StageGraph, worker: &mut WorkerState) -> bool {
    try_push_held_consensus(graph, worker)
}

/// Public wrapper for [`try_push_held_filter`].
pub fn try_push_held_filter_pub(graph: &StageGraph, worker: &mut WorkerState) -> bool {
    try_push_held_filter(graph, worker)
}

/// Public wrapper for [`try_push_held_compress`].
pub fn try_push_held_compress_pub(graph: &StageGraph, worker: &mut WorkerState) -> bool {
    try_push_held_compress(graph, worker)
}
