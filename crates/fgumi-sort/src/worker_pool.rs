...
struct SortWorkerState {
    ...
    idle_iter: u64,
    /// How long the wait this worker just took actually lasted, in nanoseconds,
    /// if the previous loop iteration waited. Taken (and cleared) by the next
    /// iteration that finds work, which is what makes that wait "productive" —
    /// see [`crate::merge_stalls::WakeLatencyStats`].
    ///
    /// The *observed* duration rather than the backoff level that was requested:
    /// waits are `park_timeout`, so a wake from [`SharedPipelineState::wake_one_worker`]
    /// ends one early, and the gap between requested and actual is precisely
    /// what that wake exists to create.
    last_wait_nanos: Option<u64>,
}
    ...
    fn worker_loop(
        ...
        loop {
            ...
            if did_work {
                ...
                // sleeps alike.
                if let Some(waited_ns) = worker.last_wait_nanos.take() {
                ...
                worker.last_wait_nanos = Some(idle_ns);
            }
        }
    }
