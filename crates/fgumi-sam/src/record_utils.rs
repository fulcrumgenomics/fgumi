...
pub fn num_bases_extending_past_mate(read: &RecordBuf) -> usize {
    ...
    if is_positive_strand {
        ...
        if read_alignment_start_signed <= mate_unclipped_start_pos {
            ...
            let lookup_pos = if mate_unclipped_start_pos < 0 {
                0
            } else {
                #[expect(
                    clippy::cast_sign_loss,
                    reason = "value is verified non-negative by the if-check above"
                )]
                let pos = mate_unclipped_start_pos as usize;
                pos
            };
            ...
            #[expect(
                clippy::cast_possible_wrap,
                reason = "soft clipping lengths are small relative to isize::MAX"
            )]
            ...
            if leading_soft_clip > gap {
                #[expect(
                    clippy::cast_sign_loss,
                    reason = "result is guaranteed positive by the if-check above"
                )]
                let result = (leading_soft_clip - gap) as usize;
                result
            } else {
                0
            }
...
pub fn get_pair_orientation(record: &RecordBuf) -> PairOrientation {
    ...
    // Genomic positions always fit in i64
    #[expect(
        clippy::cast_possible_wrap,
...
pub fn mate_unclipped_start_raw(bam: &[u8]) -> Option<isize> {
    ...
    let leading = parse_leading_clips(mc_cigar);
...
/// Parses leading soft+hard clips from a CIGAR string.
///
/// Used internally by [`mate_unclipped_start`] and [`mate_unclipped_start_raw`].
fn parse_leading_clips(cigar_str: &str) -> usize {
    let ops = parse_cigar_string(cigar_str);
    leading_clipping(&ops)
}
