...
/// Wrapper to test clap parsing of generated Multi struct args.
#[derive(Parser, Debug)]
...
#[test]
fn test_multi_options_clap_parsing() {
    let cli = TestCli::try_parse_from(["test-cli", "--test::name", "parsed", "--test::count", "7"])
        .unwrap();
    let opts = cli.multi.validate().unwrap();
    assert_eq!(opts.name, "parsed");
    assert_eq!(opts.count, 7);
    assert_eq!(opts.output, None);
}

#[test]
fn test_multi_options_clap_rejects_missing_required() {
    // --test::name is required, so omitting it should fail parsing validation
    let cli = TestCli::try_parse_from(["test-cli"]).unwrap();
    assert!(cli.multi.validate().is_err());
}
