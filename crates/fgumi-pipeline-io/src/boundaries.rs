...
///
/// Returns:
/// - `Some(offset)` — `offset` bytes consume the complete header; the first record starts there.
/// - `Some(0)` — `data` does not begin with the BAM magic; treat records as starting at offset 0.
/// - `None` — `data` is too short to contain a complete header; more bytes are needed.
#[must_use]
pub fn bam_header_len(data: &[u8]) -> Option<usize> {
...
impl BoundaryState {
    ...
    pub fn scan(
        ...
        if !self.header_skipped {
            if let Some(header_size) = bam_header_len(&self.work_buffer) {
                cursor = header_size;
...
mod tests {
    ...
    fn bam_header_len_matches_constructed_header() {
        ...
        assert_eq!(bam_header_len(&h), Some(12));
        ...
        assert_eq!(bam_header_len(&h2), Some(12 + 4 + 3 + 4));

        // Non-BAM magic → Some(0) (records start immediately).
        let non_bam = b"NOT\x01\x00\x00\x00\x00\x00\x00\x00\x00";
        assert_eq!(bam_header_len(non_bam), Some(0));

        // Too short to hold a complete header → None.
        assert_eq!(bam_header_len(b"BAM"), None);
        assert_eq!(bam_header_len(&[]), None);
        ...
        // data.len() == 8, need offset+4 == 12 → None
        assert_eq!(bam_header_len(&short), None);
    }
}
