...
//!
//! `SortBuffer` (`Serial`) ingests `RecordBatch`es into an in-memory
//! [`CoordinateChunkSorter`], par-sorts each filled buffer, and emits the sorted
//! records as a [`SortChunkEvent`] — **without touching disk**. Mid-stream spill
...
//! end), so peak memory stays bounded: the staged-events queue is drained before
//! the next batch is ingested, holding at most ~one spill chunk plus the live
//! buffer.
//!
...
impl ChunkSorter {
    ...
    fn from_sorter(sorter: RawExternalSorter, header: &Header) -> Result<Self> {
        ...
        let sort_threads = sorter.num_threads().max(1);
