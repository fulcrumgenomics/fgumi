...
//!
//! All steps are currently **unwired** — no chain references them — so they have no
//! effect on pipeline behaviour.  They will be wired in the next increment.
...
impl Step for ReadBlocks {
    ...
    fn try_run(&mut self, ctx: &mut StepCtx<'_, Self>) -> io::Result<StepOutcome> {
        ...
        // 5. Try to pop and admit one BgzfBlock.
        if let Some(block) = ctx.input.pop() {
            // A seal is armed from the previous block hitting the budget.  The
            ...
            // be completed by the run this block opens).
            ...
            // Check if we hit the run budget; arm the deferred seal.
            if self.run_cumulative >= self.run_cap {
                self.seal_pending = true;
            }
            return Ok(StepOutcome::Progress);
