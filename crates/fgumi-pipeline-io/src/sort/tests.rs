...
/// `SortMerge` → `VecSink`) and collect the merged record bytes. Coordinate-only
/// (matching `SortBuffer`'s current support).
#[allow(clippy::too_many_arguments)]
fn drive_sort_buffer_pipeline(
    ...
    // sorted records, so any codec yields output parity; use the production
...
/// multi-spill coordinate workload, pinning that `run_detached_step` preserves
