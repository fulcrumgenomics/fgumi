...
mod tests {
    ...
    /// Both ends of a template share a read name, so the raw-record cap keeps or drops them
    /// together instead of splitting the template across ends.
    #[test]
    fn downsample_reads_keeps_both_ends_of_a_template() {
        let options =
            VanillaUmiConsensusOptions { min_reads: 1, max_reads: Some(4), ..Default::default() };
        let caller = VanillaUmiConsensusCaller::new("c".to_string(), "A".to_string(), options);
        ...
        let kept = caller.downsample_reads(reads);
        let mut counts: HashMap<String, usize> = HashMap::new();
        for r in &kept {
            let name = String::from_utf8(RawRecordView::new(r.as_ref()).read_name().to_vec())
                .expect("read names are ASCII");
        ...
        assert!(
            ...
            "every retained template must contribute both ends, got {counts:?}"
        );
    }
    ...
    /// `create_source_read` must stamp the fgbio rank of the record's read name, so
    /// downsampling never re-hashes and mates (which share a name) share a rank.
    #[rstest]
    #[case::q0("q0", -593_808_727)]
    #[case::read0("read0", 916_970_908)]
    fn create_source_read_stamps_the_fgbio_rank(#[case] name: &str, #[case] expected: i32) {
        let caller = VanillaUmiConsensusCaller::new(
            "consensus".to_string(),
            "A".to_string(),
            VanillaUmiConsensusOptions { min_reads: 1, ..Default::default() },
        );
