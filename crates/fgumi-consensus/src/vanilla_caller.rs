...
impl VanillaUmiConsensusCaller {
    ...
    fn annotate_methylation(
        ...
        // Get ref_id and alignment info from first source read
        let first = source_reads.first()?;
        if first.ref_id < 0 || first.alignment_start < 0 {
            return None;
        }
        let ref_name = ref_names.get(usize::try_from(first.ref_id).ok()?)?;

        // Determine strand from source reads
        let is_top = methylation::is_top_strand(first.flags);

        // Map consensus positions to reference positions
        let ref_positions = methylation::query_to_ref_positions(
            &first.simplified_cigar,
            first.alignment_start,
            first.flags & fgumi_raw_bam::flags::REVERSE != 0,
            &first.original_cigar,
        );
...
mod tests {
    ...
    /// Test: All reads show T at ref-C → unmethylated, consensus base restored to C.
    #[test]
    fn test_simplex_em_seq_all_unmethylated() {
        ...
        // Consensus bases should be restored to C (the reference base)
        assert_eq!(consensus.bases, vec![b'C'; 10]);

        // MM tag should be present with methylation probabilities near 0
        let mm = consensus.get_string_tag(b"MM").expect("MM tag should be present");
        assert!(mm.starts_with(b"C+m"), "MM should start with C+m");

        let ml = consensus.get_u8_array_tag(b"ML").expect("ML tag should be present");
        for &p in &ml {
            assert_eq!(p, 0, "Expected methylation probability 0 for fully unmethylated");
        }
    ...
    /// TAPs: All reads show T at ref-C → methylated. MM prob should be 255.
    /// Note: consensus base stays as T (not restored to C) since methylation
    /// evidence is preserved in cu/ct/MM/ML tags, and downstream alignment
    /// needs to see the converted base.
    #[test]
    fn test_simplex_taps_all_methylated() {
        ...
        // Consensus base stays as T — not restored to C
        assert_eq!(consensus.bases, vec![b'T'; 10]);

        // MM tag should NOT be present — consensus base is T, not C, so no C+m entries
        assert!(
            consensus.get_string_tag(b"MM").is_none(),
            "MM tag should be absent when consensus is T"
        );
    ...
    /// TAPs: Mixed — 2 reads T (methylated), 1 read C (unmethylated). Prob = 2/3 ≈ 170.
    /// Consensus base is C (majority of reads that count toward consensus).
    #[test]
