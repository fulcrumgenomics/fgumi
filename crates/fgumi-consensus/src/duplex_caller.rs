...
impl DuplexConsensusCaller {
    ...
    fn group_by_mi_and_strand(
        ...
        for read in reads {
            // Extract MI tag bytes from the raw record
            let Some(mi_bytes) = RawRecordView::new(&read).tags().find_string(b"MI") else {
                // No MI tag, skip this read
                continue;
            };
            let mi = String::from_utf8(mi_bytes.to_vec()).context("MI tag is not valid UTF-8")?;
    ...
    fn process_group(
        ...
        let ab_r1s: Vec<&RawRecord> =
            a_records.iter().filter(|r| r.flags() & flags::FIRST_SEGMENT != 0).collect();
        let ab_r2s: Vec<&RawRecord> =
            a_records.iter().filter(|r| r.flags() & flags::FIRST_SEGMENT == 0).collect();
        let ba_r1s: Vec<&RawRecord> =
            b_records.iter().filter(|r| r.flags() & flags::FIRST_SEGMENT != 0).collect();
        let ba_r2s: Vec<&RawRecord> =
            b_records.iter().filter(|r| r.flags() & flags::FIRST_SEGMENT == 0).collect();
