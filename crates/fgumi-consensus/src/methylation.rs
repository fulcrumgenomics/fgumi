...
impl MethylationAnnotation {
    ...
    pub fn truncate(&self, len: usize) -> Self {
        Self { evidence: self.evidence[..len].to_vec() }
    }
}
...
pub fn query_to_ref_positions(
    ...
    let query_len: usize =
        simplified_cigar.iter().filter(|(k, _)| consumes_query(*k)).map(|(_, len)| *len).sum();
    ...
    if is_reverse {
        ...
        let ref_span: i64 = original_cigar
            .iter()
            .filter(|(k, _)| consumes_reference(*k))
...
/// Whether a CIGAR op consumes query bases.
...
/// Whether a CIGAR op consumes reference bases.
    ...
    for (i, ev) in evidence.iter_mut().enumerate() {
        ...
        for sr in source_reads {
            ...
            if base == unconverted_base {
                ev.unconverted_count += 1;
            } else if base == converted_base {
                ev.converted_count += 1;
            }
        }
...
///
/// For top-strand reads, we track `C+m` modifications.
...
pub fn build_mm_ml_tags(
        ...
        if ev.is_ref_c {
            ...
            if total > 0 {
                ...
                let numerator = match methylation_mode {
                    crate::MethylationMode::EmSeq => i32::from(ev.unconverted_count),
                    crate::MethylationMode::Taps => i32::from(ev.converted_count),
                ...
                let prob = (numerator * 255 / total).clamp(0, 255) as u8;
    ...
    // Build MM string: "C+m,skip1,skip2,...;"
    let base_char = if is_top_strand { 'C' } else { 'G' };
    let mut mm = format!("{base_char}+m");
    for s in &skips {
        use std::fmt::Write;
        write!(mm, ",{s}").unwrap();
    }
...
pub fn combine_methylation_annotations(
    ...
    for i in 0..len {
        ...
        let unconverted =
            ab_ev.map_or(0, |e| e.unconverted_count) + ba_ev.map_or(0, |e| e.unconverted_count);
        let converted =
            ab_ev.map_or(0, |e| e.converted_count) + ba_ev.map_or(0, |e| e.converted_count);
