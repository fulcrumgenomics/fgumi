# Changelog

All notable changes to this project will be documented in this file.

## [0.7.0] - 2026-08-22

### Bug Fixes

- Emit the five codec stats rows fgbio reports ([#754](https://github.com/fulcrumgenomics/fgumi/pull/754))
- Read out-of-range overlap boundaries the way fgbio does ([#755](https://github.com/fulcrumgenomics/fgumi/pull/755))
- Reconcile single-strand rejections with --stats and --rejects ([#758](https://github.com/fulcrumgenomics/fgumi/pull/758))
- Route per-read rejections to the --rejects output ([#756](https://github.com/fulcrumgenomics/fgumi/pull/756))
- Clip overlapping reads by query distance, not reference distance ([#759](https://github.com/fulcrumgenomics/fgumi/pull/759))
- Clamp the duplex window to the region both strands align ([#767](https://github.com/fulcrumgenomics/fgumi/pull/767))
- Store the CODEC fixture's reverse read in reference orientation ([#768](https://github.com/fulcrumgenomics/fgumi/pull/768))
- Count and route reads dropped by downsampling ([#724](https://github.com/fulcrumgenomics/fgumi/pull/724)) ([#785](https://github.com/fulcrumgenomics/fgumi/pull/785))
- [**breaking**] Count and route reads that trim to zero length ([#793](https://github.com/fulcrumgenomics/fgumi/pull/793))
- [**breaking**] Cap simplex --max-reads per end, not per group ([#723](https://github.com/fulcrumgenomics/fgumi/pull/723)) ([#812](https://github.com/fulcrumgenomics/fgumi/pull/812))
- Clip past-mate reads by query distance, not reference distance ([#760](https://github.com/fulcrumgenomics/fgumi/pull/760)) ([#840](https://github.com/fulcrumgenomics/fgumi/pull/840))
- Clip dovetail FR overlaps and make the TLEN FR check crate-internal ([#857](https://github.com/fulcrumgenomics/fgumi/pull/857))

### Performance

- Avoid three per-record heap allocations (group / downsample / overlapping consensus) ([#832](https://github.com/fulcrumgenomics/fgumi/pull/832))
- Iterate raw CIGAR ops without a per-record Vec at four hot sites ([#834](https://github.com/fulcrumgenomics/fgumi/pull/834))

### Testing

- Assert the bases the CODEC consensus tests are named for ([#770](https://github.com/fulcrumgenomics/fgumi/pull/770))
- Make the CODEC tests that cannot fail fail ([#774](https://github.com/fulcrumgenomics/fgumi/pull/774))
- Assert the bases the codec_template consensus tests are named for ([#808](https://github.com/fulcrumgenomics/fgumi/pull/808))

<!-- generated by git-cliff -->

## [0.6.0] - 2026-08-14

### Bug Fixes

- Count consensus reads, not templates, in --threads mode ([#681](https://github.com/fulcrumgenomics/fgumi/pull/681))
- Restore single-strand consensus via --min-reads 1,1,0 ([#683](https://github.com/fulcrumgenomics/fgumi/pull/683))
- [**breaking**] Make consensus read downsampling deterministic ([#727](https://github.com/fulcrumgenomics/fgumi/pull/727))
- [**breaking**] Downsample after filtering to the most common alignment ([#731](https://github.com/fulcrumgenomics/fgumi/pull/731))
- [**breaking**] Require mapped reads for consensus unless --allow-unmapped ([#737](https://github.com/fulcrumgenomics/fgumi/pull/737))

### Testing

- Pin record_consensus_pair to two emitted reads ([#694](https://github.com/fulcrumgenomics/fgumi/pull/694))

<!-- generated by git-cliff -->

## [0.5.0] - 2026-07-28

### Bug Fixes

- Match fgbio FilterConsensusReads parity (FILT-01/02/03/04)
- Require both aD and bD to treat a read as duplex ([#495](https://github.com/fulcrumgenomics/fgumi/pull/495))
- Symmetric per-pair FR classification + NotPrimaryFrPair reject metric (CODEC-01, R2-CODEC-01) ([#505](https://github.com/fulcrumgenomics/fgumi/pull/505))
- Panic in aOrNotB/ln_a_minus_b when a < b (R2-NUM-01) ([#507](https://github.com/fulcrumgenomics/fgumi/pull/507))
- Sort-order + error-rate guards and duplex rejection rows (CONS-01, DUP-01, R2-MET-05, R2-UCC-01) ([#508](https://github.com/fulcrumgenomics/fgumi/pull/508))
- Model codec-specific fgbio rejection reasons and seed their KV rows ([#516](https://github.com/fulcrumgenomics/fgumi/pull/516))
- Count masked bases only for retained primary reads (FILT-05) ([#523](https://github.com/fulcrumgenomics/fgumi/pull/523))
- Skip -inf lanes in ln_sum_exp_array to match fgbio's or() (SIMPLEX3-01) ([#524](https://github.com/fulcrumgenomics/fgumi/pull/524))
- [**breaking**] Clamp codec/duplex scalar depth+error tags to fgbio's Short ceiling ([#552](https://github.com/fulcrumgenomics/fgumi/pull/552))
- Apply --max-reads-per-strand cap, share combined-error Short cap, diagnose mi5252 (DUPLEX3-01/04) ([#563](https://github.com/fulcrumgenomics/fgumi/pull/563))
- Align reject-reason labels with fgbio wording (CODEC3-08) ([#566](https://github.com/fulcrumgenomics/fgumi/pull/566))
- Soft-only overlap-clip boundary, MC-independent codec clip, fgbio-faithful quality masking ([#533](https://github.com/fulcrumgenomics/fgumi/pull/533))
- Collapse codec masking if into a let-chain ([#590](https://github.com/fulcrumgenomics/fgumi/pull/590))
- Error on an over-long consensus read name instead of panicking ([#611](https://github.com/fulcrumgenomics/fgumi/pull/611))
- Derive the unanimous fast-path bound from the pre-UMI rate ([#634](https://github.com/fulcrumgenomics/fgumi/pull/634))
- Error on an over-long read name instead of panicking ([#629](https://github.com/fulcrumgenomics/fgumi/pull/629))
- [**breaking**] Restore fgbio-identical near-tie calls by default ([#666](https://github.com/fulcrumgenomics/fgumi/pull/666))

### Documentation

- Fix stale doctest crate paths in consensus/umi and gate doctests in CI ([#573](https://github.com/fulcrumgenomics/fgumi/pull/573))
- Fix stale paths in module examples and declare them non-code ([#579](https://github.com/fulcrumgenomics/fgumi/pull/579))
- Fix broken intra-doc links and gate rustdoc in CI ([#574](https://github.com/fulcrumgenomics/fgumi/pull/574))

### Features

- Accept uncompressed SAM and stdin anywhere BAM is accepted ([#644](https://github.com/fulcrumgenomics/fgumi/pull/644))

### Miscellaneous Tasks

- [**breaking**] Honest MSRV in lockstep with the toolchain + let-chain adoption ([#575](https://github.com/fulcrumgenomics/fgumi/pull/575))

### Performance

- Remove three per-block and per-read allocations ([#614](https://github.com/fulcrumgenomics/fgumi/pull/614))
- Memoize the unanimous fast-path bound per pre-UMI rate ([#635](https://github.com/fulcrumgenomics/fgumi/pull/635))
- Resolve unanimous positions via a margin-gated gap table ([#652](https://github.com/fulcrumgenomics/fgumi/pull/652))
- Memoize post-UMI adjusted probability tables ([#626](https://github.com/fulcrumgenomics/fgumi/pull/626))
- Collapse ln_sum_exp to one addition when the operands are equal ([#637](https://github.com/fulcrumgenomics/fgumi/pull/637))

### Refactor

- Share one consensus read-name implementation ([#628](https://github.com/fulcrumgenomics/fgumi/pull/628))

### Testing

- Pin that the per-base error count stays exact on deep pileups ([#613](https://github.com/fulcrumgenomics/fgumi/pull/613))

<!-- generated by git-cliff -->

## [0.4.0] - 2026-06-20

### Bug Fixes

- Clamp consensus depth at i16::MAX instead of wrapping u16 ([#448](https://github.com/fulcrumgenomics/fgumi/pull/448))

<!-- generated by git-cliff -->

## [0.3.1] - 2026-06-17

<!-- generated by git-cliff -->

## [0.3.0] - 2026-06-10

### Bug Fixes

- Gate methylation-dependent filter fns behind simplex ([#313](https://github.com/fulcrumgenomics/fgumi/pull/313))

### Refactor

- Validate and consolidate SAM tag identifiers via SamTag ([#316](https://github.com/fulcrumgenomics/fgumi/pull/316))
- Typed CodecConsensusError replaces substring matching ([#342](https://github.com/fulcrumgenomics/fgumi/pull/342))
- Delegate ConsensusOutput heap-size estimate to owning type ([#368](https://github.com/fulcrumgenomics/fgumi/pull/368))

<!-- generated by git-cliff -->

## [0.2.0] - 2026-04-22

### Bug Fixes

- Preserve BA-only orientation for methylation tags ([#298](https://github.com/fulcrumgenomics/fgumi/pull/298))
- Stream rejects to disk in simplex/duplex/codec/correct ([#293](https://github.com/fulcrumgenomics/fgumi/pull/293))

### Features

- Add EM-Seq methylation-aware consensus calling ([#168](https://github.com/fulcrumgenomics/fgumi/pull/168))
- Add EM-Seq methylation filters and performance improvements ([#169](https://github.com/fulcrumgenomics/fgumi/pull/169))
- Add --restore-unconverted-bases flag for EM-seq ([#170](https://github.com/fulcrumgenomics/fgumi/pull/170))
- Add --methylation-mode for EM-Seq/TAPs consensus ([#172](https://github.com/fulcrumgenomics/fgumi/pull/172))
- Add --methylation-mode to filter for conversion fraction ([#173](https://github.com/fulcrumgenomics/fgumi/pull/173))
- Add --methylation-mode to simulate subcommands ([#263](https://github.com/fulcrumgenomics/fgumi/pull/263))
- Additive API surface ([#288](https://github.com/fulcrumgenomics/fgumi/pull/288))

### Refactor

- Migrate fgumi to new raw-bam view/editor API ([#279](https://github.com/fulcrumgenomics/fgumi/pull/279))
- Migrate internals to RawRecord; drop RecordBuf variants ([#291](https://github.com/fulcrumgenomics/fgumi/pull/291))
- Migrate commands, storage, and pipeline to raw-byte records ([#292](https://github.com/fulcrumgenomics/fgumi/pull/292))
- Rename duplex-filter tier locals to best/worst ([#299](https://github.com/fulcrumgenomics/fgumi/pull/299))
- Migrate workspace tests to fgumi_raw_bam SamBuilder ([#295](https://github.com/fulcrumgenomics/fgumi/pull/295))
- [**breaking**] Delete vendored bam_codec; migrate simulate; propagate all remaining 48-commit work ([#296](https://github.com/fulcrumgenomics/fgumi/pull/296))

<!-- generated by git-cliff -->

## [0.1.3] - 2026-04-11

### Documentation

- Update bam_codec comments to reflect noodles#364 closure ([#182](https://github.com/fulcrumgenomics/fgumi/pull/182))

### Refactor

- Replace all unwrap() calls in crates/ with expect() ([#225](https://github.com/fulcrumgenomics/fgumi/pull/225))
- Introduce SamTag newtype for two-character BAM tag fields ([#241](https://github.com/fulcrumgenomics/fgumi/pull/241))

<!-- generated by git-cliff -->

## [0.1.2] - 2026-03-06

### Bug Fixes

- Expand ci-fmt and ci-lint to all workspace crates ([#162](https://github.com/fulcrumgenomics/fgumi/pull/162))

### Miscellaneous Tasks

- Lockstep workspace versions and fix publish workflow ([#132](https://github.com/fulcrumgenomics/fgumi/pull/132))
- Bump noodles in the all-cargo-deps group ([#131](https://github.com/fulcrumgenomics/fgumi/pull/131))

### Refactor

- Remove dead code and unify duplicated logic in fgumi-consensus ([#139](https://github.com/fulcrumgenomics/fgumi/pull/139))
- Precompute phred threshold and eliminate redundant error-counting pass ([#148](https://github.com/fulcrumgenomics/fgumi/pull/148))

<!-- generated by git-cliff -->

## [0.1.1] - 2026-02-27

### Miscellaneous Tasks

- Bump the all-cargo-deps group with 6 updates ([#120](https://github.com/fulcrumgenomics/fgumi/pull/120))

<!-- generated by git-cliff -->
