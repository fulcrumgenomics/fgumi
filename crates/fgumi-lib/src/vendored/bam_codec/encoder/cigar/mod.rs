    ...
    C: Cigar,
{
    ...
    if let Ok(op_count) = u16::try_from(cigar.len()) {
        ...
        Ok(Some([Op::new(Kind::SoftClip, k), Op::new(Kind::Skip, m)].into_iter().collect()))
    }
}
...
/// Fast path: encode CIGAR operations from a slice (no dynamic dispatch).
///
/// This bypasses the trait-based iterator and works directly with the
/// underlying slice of operations.
#[inline]
...
mod tests {
    ...
    fn test_overflowing_write_cigar_op_count() -> io::Result<()> {
        ...
        assert_eq!(
            overflowing_cigar,
            Some([Op::new(Kind::SoftClip, 4), Op::new(Kind::Skip, 1 << 16)].into_iter().collect())
        );
    ...
    fn test_write_cigar() -> Result<(), Box<dyn std::error::Error>> {
        ...
        t(&mut buf, &[Op::new(Kind::Match, 4)].into_iter().collect(), &[0x40, 0x00, 0x00, 0x00])?;
        t(
            &mut buf,
            &[Op::new(Kind::Match, 4), Op::new(Kind::HardClip, 2)].into_iter().collect(),
