...
pub fn filter_template_raw(
    ...
    let num_primary_reads = u64::from(raw_r1.is_some()) + u64::from(raw_r2.is_some());
    ...
    if !filter_template_raw_flags(raw_r1, raw_r2, config, metrics, num_primary_reads) {
        return false;
    }

    // Phase 2: Tag-based checks (MQ + UMI in one aux scan per read)
    filter_template_raw_tags(raw_r1, raw_r2, config, metrics, num_primary_reads)
}
