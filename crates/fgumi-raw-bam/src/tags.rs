...
impl TagValue<'_> {
    /// Returns the canonical BAM type byte for this variant.
    ///
...
impl<'a> RawTagsView<'a> {
    ...
    pub fn get(&self, tag: &[u8; 2]) -> Option<TagValue<'a>> {
        ...
        let aux = self.0;
        match val_type {
            b'A' => Some(TagValue::Char(aux[start])),
    ...
    pub fn iter_typed(&self) -> impl Iterator<Item = ([u8; 2], TagValue<'a>)> + 'a {
        ...
        RawTagsView::new(aux)
            .iter()
            .filter_map(move |entry| RawTagsView::new(aux).get(&entry.tag).map(|v| (entry.tag, v)))
    }
}
