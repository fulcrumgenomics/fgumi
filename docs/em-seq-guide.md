...
The EM-Seq pipeline follows the same structure as the [standard consensus pipeline](best-practice-consensus-pipeline.md), with additional flags at key steps:
...
| `filter` | `--methylation-mode em-seq`, `--min-conversion-fraction`, `--min-methylation-depth`, `--require-strand-methylation-agreement` | Methylation-specific filtering |
...
The `filter` command provides methylation-specific options. These operate on the `cu`/`ct`/`au`/`at`/`bu`/`bt` count tags emitted by `--methylation-mode em-seq` consensus calling.
...
fgumi filter --ref ref.fa --min-reads 3 --max-base-error-rate 0.1 --min-methylation-depth 3 \
  --methylation-mode em-seq --min-conversion-fraction 0.9
```
