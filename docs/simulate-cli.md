...
**Argument ordering:** `--replay-bam` must appear *before* any positional token.
Trailing positionals are captured greedily (so the command stays a drop-in for
real-aligner templates), which means a `--replay-bam` placed *after* a positional
is swallowed as an ignored token and the command fails with a missing-argument
error. The `--aligner::command` example above already uses the correct order.
