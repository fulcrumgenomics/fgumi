...
impl Runall {
    ...
    pub(super) fn sort_and_process_records(
        ...
        // --stop-after zipper: write the merged records directly (unsorted)
        if self.stop_after == StopAfter::Zipper {
            ...
            writer.finish().context("Failed to finish zippered BAM writer")?;
            info!("Zipper complete (stopped after zipper): {count} records written");
