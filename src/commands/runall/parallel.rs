...
/// Minimum thread count for the parallel pipeline to outperform the serial path.
/// Below this threshold, queue overhead exceeds the parallelism benefit.
...
impl Runall {
    pub(super) fn should_use_parallel_pipeline(&self) -> bool {
        ...
        // so MIN_PARALLEL_THREADS threads is sufficient for the parallel path
        // to outperform serial.
        let enough_threads = self.threads >= MIN_PARALLEL_THREADS;
        let stop_ok = matches!(self.stop_after, StopAfter::Consensus | StopAfter::Filter);
        enough_threads && stop_ok
    }

    /// Build a [`FilterConfig`] from the CLI filter options for the current consensus mode.
    ///
    /// Uses the same defaults as the standalone `fgumi filter` command for error rate
    /// and no-call fraction thresholds.
    pub(super) fn build_filter_config(&self) -> FilterConfig {
        ...
        FilterConfig::for_single_strand(
            ...
            None,
            fgumi_lib::defaults::FILTER_MAX_NO_CALL_FRACTION,
        )
    }
    ...
    pub(super) fn build_pipeline_config_from_header(
        ...
        Ok(PipelineConfig {
            ...
            require_ss_agreement: self.filter_opts.filter_require_ss_agreement,
