...
impl Runall {
    ...
    pub(super) fn run_from_group(
        ...
        for result in mi_group_iter {
            ...
            if output.count > 0 {
                total_consensus_reads += output.count;
                writer.write_raw_bytes(&output.data).context("Failed to write consensus reads")?;
            }

            progress.log_if_needed(output.count as u64);
        }
