...
impl Runall {
    ...
    /// Validate that the UMI tag is exactly 2 characters.
    fn validate_umi_tag(&self) -> Result<()> {
    ...
    pub(crate) fn validate(&self) -> Result<()> {
        ...
        match self.start_from {
            ...
            StartFrom::Extract => {
                ...
                // the fastq stage.
                let stops_before_align =
                    matches!(self.stop_after, StopAfter::Extract | StopAfter::Fastq);
                if !stops_before_align {
                    self.require_reference()?;
                    self.require_aligner_command()?;
                }
