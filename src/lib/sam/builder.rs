...
//!
//! ```rust
//! use fgumi_lib::sam::builder::{RecordBuilder, ConsensusTagsBuilder};
//!
...
use super::to_smallest_signed_int;
use anyhow::Result;
use bstr::BString;
...
///
/// ```rust
/// use fgumi_lib::sam::builder::{SamBuilder, Strand};
///
    ...
    pub fn build(self) -> (RecordBuf, RecordBuf) {
        ...
        *r1.flags_mut() = flags1;
        ...
        if !unmapped1 {
            ...
            *r1.alignment_start_mut() = Some(Position::try_from(self.start1.unwrap()).unwrap());
            *r1.cigar_mut() = parse_cigar(&cigar1).into_iter().collect();
            *r1.mapping_quality_mut() = Some(
                noodles::sam::alignment::record::MappingQuality::try_from(self.mapq1).unwrap(),
            );
        }

        if !unmapped2 {
            *r1.mate_reference_sequence_id_mut() = Some(contig2);
        ...
        r1.data_mut().insert(rg_tag, BufValue::from(self.parent.read_group_id.clone()));

        // Add custom attributes
        for (tag_str, value) in &self.attrs {
            if tag_str.len() == 2 {
                let tag = Tag::new(tag_str.as_bytes()[0], tag_str.as_bytes()[1]);
                r1.data_mut().insert(tag, value.clone());
            }
        }
        ...
        *r2.flags_mut() = flags2;
        ...
        if !unmapped2 {
            *r2.reference_sequence_id_mut() = Some(contig2);
            *r2.alignment_start_mut() = Some(Position::try_from(self.start2.unwrap()).unwrap());
            *r2.cigar_mut() = parse_cigar(&cigar2).into_iter().collect();
            *r2.mapping_quality_mut() = Some(
        ...
        // Add read group tag
        r2.data_mut().insert(rg_tag, BufValue::from(self.parent.read_group_id.clone()));

        // Add custom attributes
        for (tag_str, value) in &self.attrs {
            if tag_str.len() == 2 {
                let tag = Tag::new(tag_str.as_bytes()[0], tag_str.as_bytes()[1]);
                r2.data_mut().insert(tag, value.clone());
            }
        }

        // Calculate template length if both mapped to same contig
        if !unmapped1 && !unmapped2 && self.contig == contig2 {
            let start1 = self.start1.unwrap() as i32;
            let start2 = self.start2.unwrap() as i32;
            let end1 = start1 + bases1.len() as i32 - 1;
            let end2 = start2 + bases2.len() as i32 - 1;

            let (left, right) = if start1 <= start2 { (start1, end2) } else { (start2, end1) };
            let tlen = right - left + 1;

            *r1.template_length_mut() = if start1 <= start2 { tlen } else { -tlen };
            *r2.template_length_mut() = if start2 <= start1 { tlen } else { -tlen };
        }
...
impl RecordPairBuilder {
    ...
    pub fn build(self) -> (RecordBuf, RecordBuf) {
        ...
        if r1_mapped && r2_mapped && self.reference_sequence_id == r2_ref_id {
            let start1 = self.r1_start.unwrap() as i32;
            let start2 = self.r2_start.unwrap() as i32;
            let end1 = start1 + r1_seq.len() as i32 - 1;
            let end2 = start2 + r2_seq.len() as i32 - 1;
...
mod tests {
    ...
    fn test_add_pair_unmapped() {
        let mut builder = SamBuilder::new_unmapped();
        let (r1, r2) = builder.add_pair().name("pair1").bases1("AAAA").bases2("CCCC").build();
    ...
    fn test_add_pair_mapped() {
        ...
        assert!(!r1.flags().is_unmapped());
        assert!(!r2.flags().is_unmapped());
        assert_eq!(r1.alignment_start(), Some(Position::try_from(100).unwrap()));
        assert_eq!(r2.alignment_start(), Some(Position::try_from(200).unwrap()));
    }

    #[test]
    fn test_add_pair_with_attrs() {
        let mut builder = SamBuilder::new_unmapped();
        let (r1, r2) = builder.add_pair().attr("MI", "test_umi").attr("RX", "ACGT").build();

        let mi_tag = Tag::new(b'M', b'I');
        assert!(r1.data().get(&mi_tag).is_some());
        assert!(r2.data().get(&mi_tag).is_some());
    }

    #[test]
    fn test_sequential_names() {
        let mut builder = SamBuilder::new_unmapped();
        let r1 = builder.add_frag().bases("ACGT").build();
        let r2 = builder.add_frag().bases("ACGT").build();
        let r3 = builder.add_frag().bases("ACGT").build();

        assert_eq!(r1.name().map(std::convert::AsRef::as_ref), Some(b"0000".as_ref()));
        assert_eq!(r2.name().map(std::convert::AsRef::as_ref), Some(b"0001".as_ref()));
        assert_eq!(r3.name().map(std::convert::AsRef::as_ref), Some(b"0002".as_ref()));
    }
    ...
    fn test_template_length_calculation() {
        ...
        assert_eq!(r1.template_length(), 14);
        assert_eq!(r2.template_length(), -14);
    }
    ...
    fn test_record_pair_builder_basic() {
        ...
        assert!(r2.flags().is_last_segment());
        assert!(!r1.flags().is_reverse_complemented());
        assert!(r2.flags().is_reverse_complemented()); // FR default
    }
    ...
    fn test_record_pair_builder_per_read_tags() {
        ...
        let r1_mq = r1.data().get(&mq_tag).expect("R1 should have MQ tag");
        let r2_mq = r2.data().get(&mq_tag).expect("R2 should have MQ tag");
        ...
        match r1_mq {
            ...
            _ => panic!("Expected integer type for MQ tag, got {r1_mq:?}"),
        }
        match r2_mq {
            ...
            _ => panic!("Expected integer type for MQ tag, got {r2_mq:?}"),
        }
    }
    ...
    fn test_record_pair_builder_secondary() {
        ...
        assert!(r1.flags().is_secondary());
        assert!(r2.flags().is_secondary());
        assert!(!r1.flags().is_supplementary());
        assert!(!r2.flags().is_supplementary());
    }
    ...
    fn test_duplex_consensus_tags() {
        ...
        assert!(record.data().get(&ad_tag).is_some());
        assert!(record.data().get(&bd_tag).is_some());
        assert!(record.data().get(&am_tag).is_some());
        assert!(record.data().get(&bm_tag).is_some());
        assert!(record.data().get(&ae_tag).is_some());
        assert!(record.data().get(&be_tag).is_some());
    }
}
...
///
/// ```rust
/// use fgumi_lib::sam::builder::cigar_seq_len;
...
///
/// ```rust
/// use fgumi_lib::sam::builder::repeat_n;
...
///
/// ```rust
/// use fgumi_lib::sam::builder::uniform_qualities;
...
///
/// ```rust
/// use fgumi_lib::sam::builder::degrading_qualities;
...
pub fn degrading_qualities(start: u8, end: u8, length: usize) -> Vec<u8> {
    ...
    (0..length).map(|i| (start_f + step * i as f64).round() as u8).collect()
}
...
///
/// ```rust,no_run
/// use fgumi_lib::sam::builder::create_default_test_fasta;
