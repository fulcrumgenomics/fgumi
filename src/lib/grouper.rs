...
impl Grouper for SingleRecordGrouper {
    ...
    fn add_records(&mut self, records: Vec<DecodedRecord>) -> io::Result<Vec<Self::Group>> {
        // Decode each raw record into a RecordBuf. Uses a default header because
        // the reference-sequence name lookup is not required for pass-through pipelines.
        let header = noodles::sam::Header::default();
        records
            .into_iter()
            .map(|d| {
                raw_record_to_record_buf(&d.into_raw_bytes(), &header).map_err(io::Error::other)
            })
            .collect()
    }
