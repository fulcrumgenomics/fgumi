...
///
/// The two standalone metrics commands each fill exactly one slot, and
/// `add_metrics`'s `duplex`-first selection would silently ignore the simplex
/// options if both were set — so a both-filled bag is rejected as a framework
/// bug rather than resolved by precedence.
///
/// The check is scoped to chains that actually contain `Stage::Metrics`:
/// `ChainSpec::single_stage` accepts any `StageOptionsBag`, whose metrics fields
/// are public, so a non-metrics chain may carry both slots without ever
/// referencing either — rejecting it here would be a spurious failure.
///
/// # Errors
///
/// Returns an error when a metrics chain fills both slots.
#[cfg(feature = "consensus")]
fn validate_metrics_slots_exclusive(spec: &ChainSpec) -> Result<()> {
    if spec.stages.contains(&Stage::Metrics)
        && spec.stage_opts.simplex_metrics.is_some()
        && spec.stage_opts.duplex_metrics.is_some()
    {
        bail!(
...
pub fn validate_cross_stage_constraints(spec: &ChainSpec) -> Result<()> {
    ...
    // Rule 5c: a `Stage::Metrics` chain fills exactly one metrics options slot.
    // Extracted so this function stays under the line cap.
    #[cfg(feature = "consensus")]
    validate_metrics_slots_exclusive(spec)?;
...
mod tests {
    ...
    fn cross_stage_metrics_rejects_both_slots_filled() {
        ...
        spec.stage_opts.simplex_metrics = Some(SimplexMetricsOptions {
        ...
        let err = validate_cross_stage_constraints(&spec).unwrap_err();
        assert!(err.to_string().contains("both simplex_metrics and duplex_metrics"), "got: {err}");
    }

    /// Rule 5c is scoped to metrics chains: a NON-metrics chain (routed via
    /// `ChainSpec::single_stage`, which accepts any `StageOptionsBag`) that
    /// happens to carry both public metrics slots must not be rejected — the
    /// both-slots guard only concerns a `Stage::Metrics` chain, whose
    /// `add_metrics` `duplex`-first precedence is what makes a both-filled bag
    /// ambiguous. A non-metrics chain never references either slot.
    #[test]
    #[cfg(feature = "consensus")]
    fn cross_stage_non_metrics_allows_both_metrics_slots_filled() {
        use crate::commands::duplex_metrics::DuplexMetricsOptions;
        use crate::commands::simplex_metrics::SimplexMetricsOptions;
        let mut spec = empty_spec(vec![Stage::Group]);
        spec.stage_opts.simplex_metrics = Some(SimplexMetricsOptions {
        ...
        validate_cross_stage_constraints(&spec)
            .expect("a non-metrics chain must not trip the metrics both-slots guard");
    }
