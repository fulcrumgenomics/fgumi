...
pub fn extract_template_and_fastq(
    ...
    let num_templates = templates.len();
    let mut accumulated_records: Vec<RecordBuf> = Vec::with_capacity(num_templates);
    ...
    for (index, tmpl_seg) in templates.iter().enumerate() {
        ...
        *record.quality_scores_mut() = QualityScores::from(qual_values);
        ...
        accumulated_records.push(record);
    }

    // Encode each RecordBuf to RawRecord. Use a minimal header (unmapped reads
    // have no reference dependency) since records are strictly unmapped here.
    let minimal_header = noodles::sam::Header::default();
    let mut encoder = fgumi_raw_bam::RecordBufEncoder::new(&minimal_header);
    let raw_records: Vec<fgumi_raw_bam::RawRecord> = accumulated_records
        .iter()
        .map(|rb| encoder.encode(rb))
...
mod tests {
    ...
    fn test_make_raw_records_simple_paired() {
        let rs = ReadStructure::from_str("+T").expect("valid read structure");
        let r1 = FastqSet::from_record_with_structure(b"@read1", b"ACGT", b"IIII", &rs, &[])
            .expect("valid FASTQ");
        let r2 = FastqSet::from_record_with_structure(b"@read1", b"TGCA", b"FFFF", &rs, &[])
            .expect("valid FASTQ");
        ...
        assert!(result.is_ok());
        let batch = result.expect("should produce raw records");
    ...
    fn test_make_raw_records_with_umi() {
        let rs_umi = ReadStructure::from_str("4M+T").expect("valid read structure");
        let rs_template = ReadStructure::from_str("+T").expect("valid read structure");
        let r1 =
    ...
    fn test_make_raw_records_no_templates_fails() {
        ...
        let rs = ReadStructure::from_str("+M").expect("valid read structure");
        let r1 = FastqSet::from_record_with_structure(b"@read1", b"ACGT", b"IIII", &rs, &[])
            .expect("valid FASTQ");
