...
mod tests {
    ...
    fn test_primary_alignment_info_from_record() {
        ...
        use noodles::sam::Header;
        use noodles::sam::alignment::record::data::field::Tag as SamTag2;

        // Create a supplementary record with pa tag
        let info = PrimaryAlignmentInfo::new(0, 100, false, 0, 200, true);
        // Build the pa tag value (Int32 array) into the raw builder
        let tag_val = info.to_tag_value();
        let mut b = RawSamBuilder::new();
        b.read_name(b"test").sequence(b"ACGT").qualities(&[30; 4]).flags(raw_flags::SUPPLEMENTARY);
        // to_tag_value() returns an Int32 array — add it via noodles after conversion
        let mut record = raw_record_to_record_buf(&b.build(), &Header::default())
            .expect("raw_record_to_record_buf failed in test");
        record.data_mut().insert(SamTag2::from(*b"pa"), tag_val);
