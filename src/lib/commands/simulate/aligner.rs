...
pub struct Aligner {
    ...
    #[arg(long = "replay-bam", value_name = "BAM")]
...
impl Command for Aligner {
    fn execute(&self, _command_line: &str) -> Result<()> {
        ...
        // The drainer owns `stdin` on its own thread, so it must be the `Send`
        // `Stdin` handle (a `StdinLock` guard is not `Send`). The emitter runs on
        // this thread, so lock stdout once and wrap it in a `BufWriter`: the
        // replay is BGZF/BAM, dense in `0x0A`, and the default line-buffered
        // `Stdout` splits nearly every 8 KiB `io::copy` chunk at its last newline
        // into an extra `write` — roughly doubling the syscall count. Block
        // buffering keeps the passthrough cheap, which matters for a tool whose
        // whole job is to add as little overhead as possible to the benchmarked
        // pipeline.
        let stdout = io::stdout();
        let writer = io::BufWriter::new(stdout.lock());
        simulate_aligner(io::stdin(), replay, writer)?;
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn streams_replay_verbatim() {
        // Asserts only that the replay bytes reach stdout verbatim; the
        // stdin-drain-under-backpressure contract is exercised by
        // `does_not_deadlock_against_a_single_threaded_write_then_read_caller`
        // (a non-draining emitter blocks its 4 MiB writer and trips the watchdog)
        // and by the subprocess integration test — an in-memory `Cursor` here
        // EOFs whether or not it is actually read, so it cannot pin draining.
