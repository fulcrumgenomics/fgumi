...
const FILTER_EXAMPLES: &str = r"EXAMPLES:
    # Filter simplex consensus reads by minimum reads and error rate
    fgumi filter -i consensus.bam -o filtered.bam \
        --ref ref.fa \
        --min-reads 3 \
        --max-read-error-rate 0.05 \
        --min-base-quality 30

    # Duplex filtering with per-strand thresholds
    fgumi filter -i duplex_consensus.bam -o filtered.bam \
        --ref ref.fa \
        --min-reads 6,3,3 \
        --max-read-error-rate 0.025 \
        --max-base-error-rate 0.1

    # Route rejected reads to a separate BAM
    fgumi filter -i consensus.bam -o filtered.bam \
        --ref ref.fa \
        --min-reads 3 \
...
///
/// Exposed `pub` so `fgumi runall`'s `FilterStage` can use the same accumulator
/// type and write identical `--stats` TSV output for equivalent CLI args.
#[derive(Default, Debug, Clone)]
pub struct CollectedFilterMetrics {
    ...
    /// Total bases masked.
    pub total_bases_masked: u64,
}

impl CollectedFilterMetrics {
    /// Fold `other` into `self`. Used both per-batch (local → slot) and at
    /// pipeline end (slot → global).
    pub fn merge(&mut self, other: Self) {
        self.total_records += other.total_records;
        self.passed_records += other.passed_records;
        self.failed_records += other.failed_records;
        self.total_bases_masked += other.total_bases_masked;
    }
}

/// Write the filter-stats TSV used by both `fgumi filter --stats` and
/// `fgumi runall --filter::stats`.
///
/// Format is a 4-row TSV: `total_reads`, `passed_reads`, `failed_reads`,
/// `pass_rate`.
///
/// # Errors
///
/// Returns an error if the file cannot be created or written.
pub fn write_filter_stats(
    path: &std::path::Path,
    total: u64,
    passed: u64,
    failed: u64,
...
impl Command for Filter {
    fn execute(&self, command_line: &str) -> Result<()> {
        ...
        if let Some(r) = &self.reference {
            info!("Reference: {}", r.display());
        } else {
            info!("Reference: <none> (tag regeneration disabled)");
        }
...
impl Filter {
    /// Build a [`FilterConfig`] from the CLI options.
    ///
    /// Exposed (pub) so the `test_defaults_parity` integration test can compare
    /// `fgumi filter` defaults against `fgumi runall` defaults (see
    /// `Runall::build_filter_config`).
    #[must_use]
    ...
    fn setup_pipeline(&self, num_threads: usize, header: &Header) -> Result<FilterPipelineSetup> {
        ...
        let config = Arc::new(self.build_filter_config());
