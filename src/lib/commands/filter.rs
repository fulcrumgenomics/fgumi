...
pub struct Filter {
    ...
    /// Per-stage filter tuning options (flattened so the standalone CLI
    /// surface is unchanged: `--min-reads`, `--max-read-error-rate`, …).
    #[command(flatten)]
...
/// Per-stage filter tuning options, flattened into both the standalone
/// `Filter` command (as bare `--min-reads`, `--ref`, … flags) and the
/// `RunAll` command (as prefixed `--filter::min-reads`,
/// `--filter::ref` flags via the `MultiFilterOptions` companion struct
/// generated by `#[multi_options]`).
///
/// `Default` must match each field's clap `default_value` exactly —
/// `MultiFilterOptions`' generated `default_value_t` reads from
/// `FilterOptions::default().field`, and clap requires the default
    ...
    fn execute(&self, command_line: &str) -> Result<()> {
        ...
        let spec = ChainSpec {
            ...
            stage_opts: StageOptionsBag {
                filter: Some(self.options.clone()),
                ..Default::default()
            },
...
mod tests {
    ...
    fn test_parameter_validation() {
        ...
        cmd.options.min_reads = vec![3];
        cmd.options.max_read_error_rate = vec![0.1];
        cmd.options.max_base_error_rate = vec![0.2];
        cmd.options.min_base_quality = Some(13);
        cmd.options.reverse_per_base_tags = true;
        cmd.options.max_no_call_fraction = 0.1;
    ...
    fn test_invalid_error_rate() {
        ...
        cmd.options.min_reads = vec![3];
        cmd.options.max_read_error_rate = vec![1.5]; // Invalid: > 1.0
        cmd.options.max_base_error_rate = vec![0.2];
        cmd.options.min_base_quality = Some(13);
        cmd.options.reverse_per_base_tags = true;
        cmd.options.max_no_call_fraction = 0.1;
    ...
    fn test_default_filter_parameters() {
        ...
        assert_eq!(filter.options.min_base_quality, Some(13));
        assert!(filter.threading.is_single_threaded());
        assert!(filter.options.filter_by_template);
    }
    ...
    fn test_filter_with_optional_outputs() {
        ...
        assert_eq!(filter.options.min_mean_base_quality, Some(20.0));
        assert_eq!(filter.options.rejects, Some(PathBuf::from("rejects.bam")));
        assert_eq!(filter.options.stats, Some(PathBuf::from("stats.txt")));
        assert!(filter.options.require_single_strand_agreement);
    }
    ...
    fn test_validate_parameters_too_many_values() {
        ...
        assert!(filter.options.validate_parameters().is_err());
    }
    ...
    fn test_validate_min_methylation_depth_too_many_values() {
        ...
        cmd.options.min_methylation_depth = vec![4, 3, 2, 1]; // Too many values
        assert!(cmd.options.validate_parameters().is_err());
    }
    ...
    fn test_validate_strand_agreement_requires_ref() {
        ...
        cmd.options.reference = None;
        cmd.options.require_strand_methylation_agreement = true;
        assert!(cmd.options.validate_parameters().is_err());
    }
    ...
    fn test_validate_conversion_fraction_out_of_range() {
        ...
        cmd.options.min_conversion_fraction = Some(1.5); // Out of range
        assert!(cmd.options.validate_parameters().is_err());
    }
    ...
    fn test_validate_conversion_fraction_requires_ref() {
        ...
        cmd.options.reference = None;
        cmd.options.min_conversion_fraction = Some(0.9);
        assert!(cmd.options.validate_parameters().is_err());
    }
    ...
    fn test_validate_conversion_fraction_requires_methylation_mode() {
        ...
        cmd.options.min_conversion_fraction = Some(0.9);
        cmd.options.methylation_mode = None;
        assert!(cmd.options.validate_parameters().is_err());
    }
    ...
    fn test_validate_max_no_call_fraction_integer() {
        ...
        let result = filter.options.validate_parameters();
    ...
    fn test_validate_max_no_call_fraction_integer_valid() {
        ...
        assert!(
            filter.options.validate_parameters().is_ok(),
    ...
    fn test_filter_with_rejects_output() {
        ...
        assert!(filter.options.rejects.is_some());
        assert_eq!(
            filter.options.rejects.expect("rejects should be set"),
            PathBuf::from("rejects.bam")
        );
    }
    ...
    fn test_filter_with_stats_output() {
        ...
        assert!(filter.options.stats.is_some());
        assert_eq!(filter.options.stats.expect("stats should be set"), PathBuf::from("stats.txt"));
    }
    ...
    fn test_filter_reverse_per_base_tags_disabled() {
        ...
        assert!(!filter.options.reverse_per_base_tags);
    }
    ...
    fn test_filter_high_min_base_quality() {
        ...
        assert_eq!(filter.options.min_base_quality, Some(30));
    }
    ...
    fn test_filter_with_min_mean_base_quality() {
        ...
        assert_eq!(filter.options.min_mean_base_quality, Some(25.0));
    }
    ...
    fn test_filter_strict_error_rates() {
        ...
        assert_eq!(filter.options.max_read_error_rate[0], 0.01);
        assert_eq!(filter.options.max_base_error_rate[0], 0.005);
        assert_eq!(filter.options.max_no_call_fraction, 0.01);
    }
    ...
    fn test_filter_lenient_error_rates() {
        ...
        assert_eq!(filter.options.max_read_error_rate[0], 0.5);
        assert_eq!(filter.options.max_base_error_rate[0], 0.5);
        assert_eq!(filter.options.max_no_call_fraction, 0.5);
    }
    ...
    fn test_filter_high_min_reads_threshold() {
        ...
        assert_eq!(filter.options.min_reads[0], 10);
    }
    ...
    fn test_filter_duplex_different_thresholds() {
        ...
        assert_eq!(filter.options.min_reads.len(), 3);
        assert_eq!(filter.options.min_reads[0], 5); // Duplex (CC)
        assert_eq!(filter.options.min_reads[1], 3); // AB
        assert_eq!(filter.options.min_reads[2], 3); // BA
    }
    ...
    fn test_filter_all_optional_outputs() {
        ...
        assert!(filter.options.stats.is_some());
        assert!(filter.options.require_single_strand_agreement);
        assert!(filter.options.min_mean_base_quality.is_some());
    }
    ...
    fn test_filter_zero_no_call_fraction() {
        ...
        assert_eq!(filter.options.max_no_call_fraction, 0.0);
    }
    ...
    fn test_filter_low_min_base_quality() {
        ...
        assert_eq!(filter.options.min_base_quality, Some(2));
    }
    ...
    fn test_validate_error_rate_ordering_ab_ba() {
        ...
        let result = cmd.options.validate_parameters();
    ...
    fn test_reverse_per_base_tags_parsing(#[case] args: &[&str], #[case] expected: bool) {
        ...
        assert_eq!(cmd.options.reverse_per_base_tags, expected);
    }
    ...
    fn test_filter_by_template_parsing(#[case] args: &[&str], #[case] expected: bool) {
        let cmd = Filter::try_parse_from(args).expect("valid CLI args should parse");
        assert_eq!(cmd.options.filter_by_template, expected);
    }
    ...
    fn test_require_single_strand_agreement_parsing(#[case] args: &[&str], #[case] expected: bool) {
        let cmd = Filter::try_parse_from(args).expect("valid CLI args should parse");
        assert_eq!(cmd.options.require_single_strand_agreement, expected);
    }
}
