...
use clap::Parser;
use fgumi_raw_bam::{
    RawRecord, extract_sequence, quality_scores_slice, read_name as raw_read_name,
};
use log::info;
...
fn write_fastq_record<W: Write>(
    ...
    // Decode sequence from 4-bit BAM encoding to ASCII bases
    *seq_buf = extract_sequence(record);
