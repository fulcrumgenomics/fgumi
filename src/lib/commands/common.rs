...
/// The command-level option groups a single-stage command copies into the
/// `#[arg(skip)]` chain-builder slots of the `<Stage>Options` it hands to
/// [`ChainSpec::single_stage`].
///
/// `simplex`, `duplex`, and `codec` each flatten these groups at the command
/// level (so they appear once on the CLI) while the chain reads them off the
/// stage options struct. Bundling the sources here — rather than letting each
/// `execute` assign the slots by hand — is what makes
/// [`FillChainSlots::fill_chain_slots`] the single wiring point.
///
/// [`ChainSpec::single_stage`]: crate::pipeline::chains::ChainSpec::single_stage
#[cfg(feature = "consensus")]
#[derive(Debug)]
pub struct ChainSlots<'a> {
    /// Input/output paths and BAM I/O tuning.
    pub io: &'a BamIoOptions,
    /// Rejected-record output settings.
    pub rejects_opts: &'a RejectsOptions,
    /// Per-run statistics output settings.
    pub stats_opts: &'a StatsOptions,
    /// Read-group and read-name-prefix settings for consensus output.
    pub read_group: &'a ReadGroupOptions,
    /// `--methylation-mode`, for the stages that support it.
    pub methylation_mode: Option<MethylationModeArg>,
    /// `--ref`, for the stages that support methylation.
    pub reference: &'a Option<PathBuf>,
}

/// Stand-in `--ref` for stages with no methylation support, so
/// [`ChainSlots::new`] has an `&Option<PathBuf>` to point at.
#[cfg(feature = "consensus")]
static NO_REFERENCE: Option<PathBuf> = None;

#[cfg(feature = "consensus")]
impl<'a> ChainSlots<'a> {
    /// Slots for a stage with no methylation support (`codec`).
    #[must_use]
    pub fn new(
        io: &'a BamIoOptions,
        rejects_opts: &'a RejectsOptions,
        stats_opts: &'a StatsOptions,
        read_group: &'a ReadGroupOptions,
    ) -> Self {
        Self {
            io,
            rejects_opts,
    ...
    /// Adds the `--methylation-mode` / `--ref` slots, for the stages whose
    /// options struct declares them (`simplex`, `duplex`).
    ///
    /// These are ignored unless the type's [`FillChainSlots`] impl was generated
    /// with `with_methylation`, so a stage cannot appear to honour `--ref` by
    /// supplying it here.
    #[must_use]
    pub fn with_methylation(
        mut self,
        methylation_mode: Option<MethylationModeArg>,
        reference: &'a Option<PathBuf>,
    ) -> Self {
        self.methylation_mode = methylation_mode;
        self.reference = reference;
        self
    }
}

/// Populates a stage options struct's `#[arg(skip)]` chain-builder slots.
///
/// Every implementation is generated by [`impl_fill_chain_slots!`] from one
/// macro body, so a slot added to the shared set is wired into `simplex`,
/// `duplex`, and `codec` at once. Before this existed each `execute` assigned
/// the slots inline, and a slot added to one command was silently missed by the
/// others — the chain then ran that stage on a `Default` its siblings did not
/// use, with no compile error and no warning.
#[cfg(feature = "consensus")]
pub trait FillChainSlots {
    /// Copies every chain-builder slot this stage carries out of `slots`.
    fn fill_chain_slots(&mut self, slots: &ChainSlots<'_>);
}

/// Implements [`FillChainSlots`] for a stage options struct.
///
/// `impl_fill_chain_slots!(FooOptions)` wires the four slots every stage
/// carries; `impl_fill_chain_slots!(FooOptions, with_methylation)` additionally
/// wires `methylation_mode` / `reference` for the stages whose options struct
/// declares them (`simplex` and `duplex`; `codec` has no methylation support).
/// The shared assignments live in this one body — that is the whole point.
///
/// The `@methylation` arm is an internal implementation detail; call the macro
/// only in the two forms above.
#[cfg(feature = "consensus")]
macro_rules! impl_fill_chain_slots {
    ($options:ty $(, $methylation:tt)?) => {
        impl $crate::commands::common::FillChainSlots for $options {
            fn fill_chain_slots(
                &mut self,
                slots: &$crate::commands::common::ChainSlots<'_>,
            ) {
                self.io.clone_from(slots.io);
                self.rejects_opts.clone_from(slots.rejects_opts);
                self.stats_opts.clone_from(slots.stats_opts);
                self.read_group.clone_from(slots.read_group);
                $(
                    $crate::commands::common::impl_fill_chain_slots!(
                        @methylation $methylation, self, slots
                    );
                )?
            }
        }
    };
    (@methylation with_methylation, $self:expr, $slots:expr) => {
        $self.methylation_mode = $slots.methylation_mode;
        $self.reference.clone_from($slots.reference);
    };
}

#[cfg(feature = "consensus")]
pub(crate) use impl_fill_chain_slots;
...
pub struct ThreadingOptions {
    ...
    /// this only sets the worker count — `codec`, `duplex`, and `simplex` all
    /// resolve an omitted flag to the same one-worker chain as `--threads 1`.
    /// `clip` is the exception: it still takes a bespoke single-threaded path
    /// when the flag is omitted, so there `--threads 1` is not a synonym for
    /// leaving it unset.
    #[arg(long = "threads")]
...
mod tests {
    ...
    /// Every `#[arg(skip)]` chain-builder slot must be populated by
    /// `fill_chain_slots`. These pin the invariant the shared helper exists to
    /// hold: a slot wired into one consensus command but missed in another used
    /// to leave that stage silently running on a `Default`.
    #[cfg(feature = "consensus")]
    mod fill_chain_slots {
        use super::super::{
            BamIoOptions, ChainSlots, FillChainSlots, MethylationModeArg, ReadGroupOptions,
            RejectsOptions, StatsOptions,
        };
        use std::path::PathBuf;

        /// Non-default values for every slot, so a slot left unwired is
        /// distinguishable from one that was copied.
        struct Sources {
            io: BamIoOptions,
            rejects_opts: RejectsOptions,
        ...
        fn sources() -> Sources {
            Sources {
                io: BamIoOptions {
                    ...
                    // Non-default so a macro that copied only the paths — or a
                    // future `BamIoOptions` field left unwired — is caught.
                    async_reader: true,
                },
                rejects_opts: RejectsOptions { rejects: Some(PathBuf::from("/rej.bam")) },
                stats_opts: StatsOptions { stats: Some(PathBuf::from("/stats.txt")) },
                read_group: ReadGroupOptions {
                    read_name_prefix: Some("pre".to_string()),
                    read_group_id: "Z".to_string(),
                },
                reference: Some(PathBuf::from("/ref.fa")),
            }
        }

        fn slots(src: &Sources) -> ChainSlots<'_> {
            ChainSlots::new(&src.io, &src.rejects_opts, &src.stats_opts, &src.read_group)
        }

        /// Asserts the four slots every stage carries were copied.
        macro_rules! assert_common_slots {
            ($options:expr, $src:expr) => {
                assert_eq!($options.io.input, $src.io.input, "io.input slot");
                assert_eq!($options.io.output, $src.io.output, "io.output slot");
                assert_eq!($options.io.async_reader, $src.io.async_reader, "io.async_reader slot");
                assert_eq!(
                    $options.rejects_opts.rejects, $src.rejects_opts.rejects,
                    "rejects slot"
                );
                assert_eq!($options.stats_opts.stats, $src.stats_opts.stats, "stats slot");
                assert_eq!(
                    $options.read_group.read_name_prefix, $src.read_group.read_name_prefix,
                    "read_group.read_name_prefix slot"
                );
                assert_eq!(
                    $options.read_group.read_group_id, $src.read_group.read_group_id,
                    "read_group.read_group_id slot"
                );
            };
        }

        #[test]
        fn simplex_fills_every_slot() {
            let src = sources();
            let mut options = crate::commands::simplex::SimplexOptions::default();
            options.fill_chain_slots(
                &slots(&src).with_methylation(Some(MethylationModeArg::EmSeq), &src.reference),
            );
            assert_common_slots!(options, src);
            assert_eq!(options.methylation_mode, Some(MethylationModeArg::EmSeq));
            assert_eq!(options.reference, src.reference);
        }

        #[test]
        fn duplex_fills_every_slot() {
            let src = sources();
            let mut options = crate::commands::duplex::DuplexOptions::default();
            options.fill_chain_slots(
                &slots(&src).with_methylation(Some(MethylationModeArg::Taps), &src.reference),
            );
            assert_common_slots!(options, src);
            assert_eq!(options.methylation_mode, Some(MethylationModeArg::Taps));
            assert_eq!(options.reference, src.reference);
        }

        /// `codec` has no methylation slots, so supplying them must not make it
        /// look as though it honours `--ref` — the common four still land.
        #[test]
        fn codec_fills_every_slot_and_ignores_methylation() {
            let src = sources();
            let mut options = crate::commands::codec::CodecOptions::default();
            options.fill_chain_slots(
                &slots(&src).with_methylation(Some(MethylationModeArg::EmSeq), &src.reference),
            );
            assert_common_slots!(options, src);
