...
fn get_core_fields_raw(raw: &RawRecord, header: &noodles::sam::Header) -> [String; 11] {
    ...
    // Quality scores in raw BAM are 0-based Phred; 0xFF means absent.
    let qual_bytes = fgumi_raw_bam::quality_scores_slice(raw.as_ref());
    let qual = if qual_bytes.is_empty() || qual_bytes[0] == 0xFF {
        "*".to_string()
    } else {
        qual_bytes.iter().map(|&q| (q + 33) as char).collect()
    };
