...
impl Review {
    ...
    fn extract_consensus_reads(
        ...
        let index = bam::bai::fs::read(self.consensus_bam.with_extension("bam.bai"))?;
    ...
    fn generate_review_file(&self, variants: &[Variant]) -> Result<()> {
        ...
        let con_index = bam::bai::fs::read(self.consensus_bam.with_extension("bam.bai"))?;
        ...
        let grp_index = bam::bai::fs::read(self.grouped_bam.with_extension("bam.bai"))?;
        ...
        for variant in variants {
            ...
            for record in consensus_reads {
                ...
                let read_base = match self.get_base_at_position(&record, variant.pos)? {
                    Some(b) => (b as char).to_ascii_uppercase(), // b is already ASCII
                    None => {
    ...
    fn has_non_reference_base(&self, record: &RawRecord, variant: &Variant) -> Result<bool> {
        ...
        if let Some(offset) = read_offset {
            ...
            if offset < l_seq {
                ...
                let code = record.get_base(offset) as usize;
                let base_char = (BAM_BASE_TO_ASCII[code] as char).to_ascii_uppercase();
