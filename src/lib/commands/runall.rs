...
//!
//! Reads an aligned BAM and produces consensus BAM in a single fused
//! pipeline, with no intermediate files. The stages active in a run are
//! selected by the required `--start-from` / `--stop-after` flags
//! (see [`RunAllStage`]):
//!
//!   * `--start-from sort` includes the in-pipeline `Sort` step (raw
//!     unsorted BAM input).
//!   * `--stop-after {simplex, duplex, codec}` selects the terminal
//!     consensus caller.
//!
//! The chain mirrors standalone `fgumi sort` (if requested) followed
//! by `fgumi group` then `fgumi {simplex,duplex,codec}`, but skips the
//! serialize-to-BAM-bytes / decompress-from-BGZF / parse-records round
//! trip between stages.
//!
//! ```text
...
/// `{Extract, Correct, AlignAndMerge, Zipper, Sort, Group, Simplex,
/// Duplex, Codec}` and the accepted stops are
...
pub struct RunAll {
    ...
    /// is set (consensus stages) and when `--start-from zipper` is used
    /// (zipper requires the FASTA + `.dict` to build the output BAM
    /// header from the reference dictionary).
    #[arg(long = "ref")]
    ...
    // ───────── aligner-side options (used only when --start-from=align-and-merge) ─────────
    ...
    /// Ignored when `--start-from` is not `align-and-merge`.
    #[command(flatten)]
    ...
    /// binary). Ignored when `--start-from` is not `align-and-merge`.
    ///
    ...
    /// `--zipper::buffer`, `--zipper::skip-pa-tags`, etc. via the
    ...
    /// Tag containing the UMI sequence.
    #[arg(short = 't', long = "raw-tag", default_value = "RX")]
    pub raw_tag: String,

    /// Tag to assign the inferred molecule ID to.
...
impl RunAll {
    ...
    /// The terminal consensus caller to run. Derived from
    /// `--stop-after`. Panics if `validate_stages` was not called
    /// first — until tasks 8-9 unblock `--stop-after sort` /
    /// `--stop-after group`, the validator rejects every
    /// non-consensus `stop_after`, so `consensus_mode()` here is
    /// `Some(_)` after a successful `validate_stages()`.
    ///
    /// The consensus algorithm for this run, from `--consensus`. Errors
    /// if absent — `--consensus` is required whenever the chain reaches
    /// the consensus stage.
...
impl Command for RunAll {
    fn execute(&self, command_line: &str) -> Result<()> {
        ...
        // `--ref` typically pairs with `--methylation-mode` for the
        // consensus stages, but several upstream start-stages also
        // legitimately need a reference without methylation OR don't
        // consume it at all (which would otherwise produce a
        // confusing "--ref requires --methylation-mode" error for a
        // ref the stage simply ignores):
        //   * `--start-from correct`    — does not use `--ref` itself
        //     today, but cross-stage chaining (correct → align → ...)
        //     in the follow-up will need it; accept it here so users
        //     can pre-stage the flag in templates.
        //   * `--start-from zipper`     — FASTA + `.dict` for the
        //     output BAM header.
        //   * `--start-from align-and-merge` — aligner reference +
        //     index files.
        // Only enforce the methylation pairing on non-upstream start
        // stages.
        let ref_is_aligner_only = matches!(
            self.start_from,
            RunAllStage::Extract
                | RunAllStage::Correct
                | RunAllStage::Zipper
        ...
        if !ref_is_aligner_only && self.reference.is_some() && self.methylation_mode.is_none() {
            bail!("--ref requires --methylation-mode to be set");
        }
        if self.raw_tag.len() != 2 {
            bail!("--raw-tag must be exactly 2 characters; got {:?}", self.raw_tag);
        }
        if self.assign_tag.len() != 2 {
            bail!("--assign-tag must be exactly 2 characters; got {:?}", self.assign_tag);
        }
...
impl RunAll {
    ...
    fn validate_align_and_merge(&self) -> Result<()> {
        let reference = self.reference.as_ref().ok_or_else(|| {
            anyhow::anyhow!(
                "--start-from align requires --ref (the aligner reference FASTA \
                 with its index files alongside)"
            )
        })?;
        ...
        if self.methylation_mode.is_some() {
            bail!(
                "--methylation-mode is not yet supported with --start-from align-and-merge. \
                 For EM-seq today, use `--aligner::command \"bwameth.py ...\"` (or \
