...
pub struct RunAll {
    ...
    /// reference dictionary), and when `--start-from align` is used (the
    /// aligner reference; see `validate_align_and_merge`). It is also accepted
    /// without `--methylation-mode` for `--start-from correct` (the
    /// `ref_is_aligner_only` exemption in `execute`), where it is not consumed.
    #[arg(long = "ref")]
