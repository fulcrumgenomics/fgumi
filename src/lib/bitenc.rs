...
impl BitEnc {
    ...
    pub fn from_bytes(seq: &[u8]) -> Option<Self> {
        ...
        Some(Self { bits, len: seq.len() as u8 })
    }
    ...
    pub fn from_umi_str(umi: &str) -> Option<Self> {
        ...
        Some(Self { bits, len: base_count as u8 })
    }
    ...
    #[inline]
    #[must_use]
    pub fn extract_bits(&self, start_base: usize, len: usize) -> u32 {
        debug_assert!(len <= 16, "Can only extract up to 16 bases into u32");
        debug_assert!(
            start_base + len <= self.len as usize,
...
mod tests {
    ...
    fn test_hamming_distance() {
        // Identical sequences
        let a = BitEnc::from_bytes(b"ACGTACGT").unwrap();
        let b = BitEnc::from_bytes(b"ACGTACGT").unwrap();
        assert_eq!(a.hamming_distance(&b), 0);

        // One difference
        let c = BitEnc::from_bytes(b"ACGTACTT").unwrap();
        assert_eq!(a.hamming_distance(&c), 1);

        // All different
        let d = BitEnc::from_bytes(b"AAAA").unwrap();
        let e = BitEnc::from_bytes(b"TTTT").unwrap();
        assert_eq!(d.hamming_distance(&e), 4);
