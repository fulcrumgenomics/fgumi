...
mod tests {
    ...
    fn test_nonexistent_sequence() {
        ...
        let result = reader.fetch(
            "chr999",
            Position::try_from(1).expect("fetching reference sequence should succeed"),
            Position::try_from(4).expect("fetching reference sequence should succeed"),
        );
    ...
    fn test_out_of_bounds() {
        ...
        let result = reader.fetch(
            "chr1",
            Position::try_from(1).expect("fetching reference sequence should succeed"),
            Position::try_from(100).expect("fetching reference sequence should succeed"),
        );
        assert!(result.is_err());
    }
