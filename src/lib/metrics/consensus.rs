...
impl ConsensusKvMetric {
    ...
    pub fn new(
        ...
        Self { key: key.into(), value: value.to_string(), description: description.into() }
    }
}
...
impl ConsensusMetrics {
    ...
    pub fn to_kv_metrics(&self) -> Vec<ConsensusKvMetric> {
        ...
        let frac_used = if self.total_input_reads == 0 {
            ...
            raw_reads_used as f64 / self.total_input_reads as f64
        };
        metrics.push(ConsensusKvMetric::new(
            "frac_raw_reads_used",
        ...
        // Rejection reasons with fgbio-compatible keys
        // Only include reasons that fgbio tracks for simplex/vanilla caller
        metrics.push(ConsensusKvMetric::new(
        ...
        // fgbio vanilla caller rejection reasons
        metrics.push(ConsensusKvMetric::new(
        ...
        // fgumi-specific rejection reasons (not in fgbio vanilla caller but useful)
        if self.rejected_insufficient_strand_support > 0 {
            metrics.push(ConsensusKvMetric::new(
...
impl super::ProcessingMetrics for ConsensusMetrics {
...
mod tests {
    ...
    fn test_consensus_kv_metric_new() {
        let kv = ConsensusKvMetric::new("test_key", 42u64, "A test metric");
