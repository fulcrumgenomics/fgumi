...
pub(super) fn read_sequence(
    ...
    let (buf, rest) = src.split_at_checked(len).ok_or(DecodeError::UnexpectedEof)?;

    *src = rest;

    let bases = buf.iter().flat_map(|&b| [decode_base(b >> 4), decode_base(b)]);
...
mod tests {
    ...
    fn test_read_sequence() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut src = &b""[..];
        assert_eq!(read_sequence(&mut src, &mut sequence, 4), Err(DecodeError::UnexpectedEof));
