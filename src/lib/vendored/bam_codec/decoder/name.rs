...
impl fmt::Display for DecodeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MissingNulTerminator { actual } => {
                write!(f, "missing NUL terminator: expected {NUL:#04x}, got {actual:#04x}")
            }
        }
    }
}
...
pub(super) fn read_name(
    ...
    let (buf, rest) = src.split_at_checked(len.get()).ok_or(DecodeError::UnexpectedEof)?;
...
mod tests {
    ...
    fn test_read_length() {
        ...
        assert_eq!(read_length(&mut src), Ok(const { NonZero::new(8).unwrap() }));

        let mut src = &[][..];
        assert_eq!(read_length(&mut src), Err(DecodeError::UnexpectedEof));

        let mut src = &0i32.to_le_bytes()[..];
        assert!(matches!(read_length(&mut src), Err(DecodeError::InvalidLength(_))));
    }
