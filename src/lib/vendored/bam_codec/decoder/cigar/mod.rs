...
pub(super) fn read_cigar(
    ...
    let (mut buf, rest) = src.split_at_checked(len).ok_or(DecodeError::UnexpectedEof)?;
...
mod tests {
    ...
    fn test_resolve() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut record = RecordBuf::builder()
            ...
            .set_cigar([Op::new(Kind::SoftClip, 4), Op::new(Kind::Skip, 8)].into_iter().collect())
            .set_sequence(Sequence::from(b"ACGT"))
            .set_data([(Tag::CIGAR, Value::Array(Array::UInt32(vec![0x40])))].into_iter().collect())
            .build();
