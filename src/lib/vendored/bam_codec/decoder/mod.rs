...
/// Decode a BAM record from raw bytes into a `RecordBuf`.
///
/// The input slice should contain the record data AFTER the 4-byte `block_size` prefix.
/// The slice reference is advanced past the consumed bytes.
///
/// # Arguments
///
/// * `src` - Mutable reference to a byte slice containing the record data.
/// * `record` - `RecordBuf` to populate with the decoded data.
///
/// # Returns
///
/// `Ok(())` on success, or a `DecodeError` if parsing fails.
...
mod tests {
    ...
    fn test_decode_with_invalid_l_read_name() {
        ...
        assert!(matches!(decode(&mut src, &mut record), Err(DecodeError::InvalidName(_))));
    }
}
