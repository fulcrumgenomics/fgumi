...
use bstr::BString;
use memchr::memchr;
use noodles::sam::alignment::{record::data::field::Type, record_buf::data::field::Value};
...
mod tests {
    ...
    fn test_read_value() -> Result<(), Box<dyn std::error::Error>> {
        ...
        t(&[b'n', b'd', b'l', b's', 0x00], Type::String, Value::from("ndls"))?;
        t(&[b'C', b'A', b'F', b'E', 0x00], Type::Hex, Value::Hex(b"CAFE".into()))?;

        t(&[b'c', 0x01, 0x00, 0x00, 0x00, 0x00], Type::Array, Value::Array(Array::Int8(vec![0])))?;
        t(&[b'C', 0x01, 0x00, 0x00, 0x00, 0x00], Type::Array, Value::Array(Array::UInt8(vec![0])))?;
        t(
