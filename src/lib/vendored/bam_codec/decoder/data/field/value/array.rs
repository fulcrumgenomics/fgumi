...
pub(super) fn get_array(src: &mut &[u8]) -> Result<Value, DecodeError> {
    ...
    let array = match subtype {
        ...
        Subtype::Int16 => {
            Array::Int16((0..len).map(|_| read_i16_le(src)).collect::<Result<_, _>>()?)
        }

        Subtype::UInt16 => {
            Array::UInt16((0..len).map(|_| read_u16_le(src)).collect::<Result<_, _>>()?)
        }

        Subtype::Int32 => {
            Array::Int32((0..len).map(|_| read_i32_le(src)).collect::<Result<_, _>>()?)
        }

        Subtype::UInt32 => {
            Array::UInt32((0..len).map(|_| read_u32_le(src)).collect::<Result<_, _>>()?)
        }

        Subtype::Float => {
            Array::Float((0..len).map(|_| read_f32_le(src)).collect::<Result<_, _>>()?)
        }
    };
