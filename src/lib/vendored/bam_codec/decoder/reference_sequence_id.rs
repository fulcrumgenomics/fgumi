...
pub(super) fn read_reference_sequence_id(src: &mut &[u8]) -> Result<Option<usize>, DecodeError> {
    ...
    match read_i32_le(src)? {
        UNMAPPED => Ok(None),
        n => usize::try_from(n).map(Some).map_err(|_| DecodeError::Invalid),
    }
}
...
mod tests {
    ...
    fn test_read_reference_sequence_id() {
        ...
        let data = [];
        let mut src = &data[..];
        assert_eq!(read_reference_sequence_id(&mut src), Err(DecodeError::UnexpectedEof));

        let data = (-2i32).to_le_bytes();
        let mut src = &data[..];
        assert_eq!(read_reference_sequence_id(&mut src), Err(DecodeError::Invalid));
    }
}
