...
///
/// Uses the global rayon thread pool for efficiency.
///
/// # Complexity
...
pub fn discover_edges_parallel_k1(
    ...
    // Use global rayon thread pool (avoids per-call pool creation overhead)
    umis.par_iter()
        .enumerate()
        .flat_map(|(i, (enc, _))| {
...
impl ParallelEditAssigner {
    ...
    /// * `_threads` - Deprecated: now uses global rayon pool
    #[must_use]
    pub fn new(max_mismatches: u32, _threads: usize) -> Self {
        Self { max_mismatches }
    }
}
...
impl UmiAssigner for ParallelEditAssigner {
    fn assign(&self, raw_umis: &[Umi]) -> Vec<MoleculeId> {
        ...
        // Discover edges in parallel (works for any k)
        let edges = discover_edges_parallel_k(&unique_umis, self.max_mismatches);

        // Build connected components using parallel union-find
...
impl UmiAssigner for ParallelAdjacencyAssigner {
    fn assign(&self, raw_umis: &[Umi]) -> Vec<MoleculeId> {
        ...
        // Phase 1: Parallel edge discovery
        let edges = discover_edges_parallel_k(&unique_umis, self.max_mismatches);
...
impl UmiAssigner for ParallelPairedAssigner {
    fn assign(&self, raw_umis: &[Umi]) -> Vec<MoleculeId> {
        ...
        // Phase 1: Parallel edge discovery
        // For paired UMIs, we use 2x the max_mismatches since errors can occur in either half
        let edges = discover_edges_parallel_k(&unique_umis, self.max_mismatches * 2);
...
mod tests {
    ...
    fn test_parallel_paired_basic() {
        ...
        match (&assignments[0], &assignments[1]) {
            (MoleculeId::PairedA(a), MoleculeId::PairedB(b)) => assert_eq!(a, b),
