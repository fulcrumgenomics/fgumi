...
impl BkTree {
    ...
    fn insert(&mut self, encoded: BitEnc, index: usize) {
        ...
        // Use raw pointer to avoid borrow checker issues during tree traversal.
        // We need to traverse down the tree while potentially modifying the current node.
        let mut current = std::ptr::from_mut(self.root.as_mut().unwrap().as_mut());

        loop {
            // SAFETY: This dereference is safe because:
            // 1. `current` is derived from `self.root` which we own exclusively via `&mut self`
            // 2. The pointer is valid: we checked `self.root.is_some()` above, and each
            //    subsequent assignment to `current` comes from a valid child reference
            // 3. No aliasing: we only traverse downward, never revisiting nodes, so there's
            //    exactly one mutable reference to each node at any time
            // 4. The loop either returns (on insert) or moves `current` to a child, ensuring
            //    we don't hold multiple references to the same node
            let node = unsafe { &mut *current };
            let dist = node.encoded.hamming_distance(&encoded);

            if let Some(child) = node.children.get_mut(&dist) {
                current = std::ptr::from_mut(child.as_mut());
            } else {
                node.children
    ...
    #[allow(unknown_lints, clippy::only_used_in_recursion, clippy::self_only_used_in_recursion)]
