...
- Count consensus reads, not templates, in --threads mode ([#681](https://github.com/fulcrumgenomics/fgumi/pull/681))
...
- Scale the per-thread memory budget by the sort phase ([#692](https://github.com/fulcrumgenomics/fgumi/pull/692))
- Compute supplementary TLEN instead of copying the mate primary's ([#684](https://github.com/fulcrumgenomics/fgumi/pull/684))
- Restore single-strand consensus via --min-reads 1,1,0 ([#683](https://github.com/fulcrumgenomics/fgumi/pull/683))
- Stream `-o -` to stdout instead of creating a file named `-` ([#689](https://github.com/fulcrumgenomics/fgumi/pull/689))
- Compare a displaced sort-key run by digest instead of aborting ([#700](https://github.com/fulcrumgenomics/fgumi/pull/700))
- [**breaking**] Make consensus read downsampling deterministic ([#727](https://github.com/fulcrumgenomics/fgumi/pull/727))
- [**breaking**] Downsample after filtering to the most common alignment ([#731](https://github.com/fulcrumgenomics/fgumi/pull/731))
- [**breaking**] Report per-reason filtered template counts ([#740](https://github.com/fulcrumgenomics/fgumi/pull/740))
- [**breaking**] Require mapped reads for consensus unless --allow-unmapped ([#737](https://github.com/fulcrumgenomics/fgumi/pull/737))

### Documentation

- Mark fgumi production ready as of v0.5.0 ([#669](https://github.com/fulcrumgenomics/fgumi/pull/669))
- Keep the binary name out of per-argument help ([#670](https://github.com/fulcrumgenomics/fgumi/pull/670))
- Name the accepted values for sort's boolean flags ([#690](https://github.com/fulcrumgenomics/fgumi/pull/690))
- Stop denying that the `pa` tag was ever written ([#682](https://github.com/fulcrumgenomics/fgumi/pull/682))
- Name the `tc` tag in comments that still say `pa` ([#695](https://github.com/fulcrumgenomics/fgumi/pull/695))
- Clarify mate tags and update aligner recommendations ([#717](https://github.com/fulcrumgenomics/fgumi/pull/717))

### Features

- Accept on/off/1/0 for boolean flags via BoolishValueParser ([#703](https://github.com/fulcrumgenomics/fgumi/pull/703))
- [**breaking**] Size the temp-file limit to the process fd budget ([#705](https://github.com/fulcrumgenomics/fgumi/pull/705))

### Miscellaneous Tasks

- Disable Miri isolation so the raw-bam proptest can run ([#701](https://github.com/fulcrumgenomics/fgumi/pull/701))
- Retarget CodeRabbit config on measured review evidence ([#675](https://github.com/fulcrumgenomics/fgumi/pull/675))
- Report scheduled-workflow failures as a tracking issue ([#702](https://github.com/fulcrumgenomics/fgumi/pull/702))
- Gate the publish list per-PR and skip on exact version ([#721](https://github.com/fulcrumgenomics/fgumi/pull/721))
- Gate PRs that touch the Miri-scoped crate ([#712](https://github.com/fulcrumgenomics/fgumi/pull/712))
- Add cargo ci-publish-order to the local gate ([#733](https://github.com/fulcrumgenomics/fgumi/pull/733))
...
- Pin reported consensus reads to records written ([#685](https://github.com/fulcrumgenomics/fgumi/pull/685))
- Reap the `cat` children the streaming tests spawn ([#716](https://github.com/fulcrumgenomics/fgumi/pull/716))
- Guard boolean-flag help across every command ([#704](https://github.com/fulcrumgenomics/fgumi/pull/704))
...
- Don't declare a stage done while its items are still in flight ([#656](https://github.com/fulcrumgenomics/fgumi/pull/656))
- Join writer threads on every exit path ([#662](https://github.com/fulcrumgenomics/fgumi/pull/662))
- [**breaking**] Restore fgbio-identical near-tie calls by default ([#666](https://github.com/fulcrumgenomics/fgumi/pull/666))
...
- Accept --index-threshold always|never, reject unsatisfiable requests ([#632](https://github.com/fulcrumgenomics/fgumi/pull/632))
...
- Remove two dead pipeline completion flags ([#657](https://github.com/fulcrumgenomics/fgumi/pull/657))
...
- Index the edit assigner's neighbour search ([#645](https://github.com/fulcrumgenomics/fgumi/pull/645))
...
- Make the SAM/stdin comparisons non-vacuous ([#663](https://github.com/fulcrumgenomics/fgumi/pull/663))
...
- Rename the template-coordinate sort-key tag written by `fgumi zipper` from `pa` to `tc` to avoid collision with bwa-mem's `pa:f` (primary-alignment score fraction) ([#268](https://github.com/fulcrumgenomics/fgumi/issues/268)). The `missing_pa_tag` dedup metric field is renamed to `missing_tc_tag`. Pre-0.2.0 fgumi-zippered BAMs carrying the legacy `pa` tag must be re-zippered before `fgumi dedup` will accept them; BAMs zippered by 0.2.0 or later carry `tc` by default and need no migration. Renaming the tag by hand is an alternative to re-zippering, but only the legacy sort key itself — the six-element `pa:B:i` array on secondary and supplementary reads — may be renamed to `tc`: bwa-mem's `pa:f` on primary reads is a different tag, and renaming it destroys alignment metadata and leaves a `tc` of the wrong type. Zippering with the skip flag (`--skip-pa-tags` before 0.5.0, `--skip-tc-tags` from 0.5.0) only stops zipper from adding a sort-key tag of its own on any release; it does not strip a tag the input already carries. Unless the input supplied one, the secondary and supplementary reads come out with no `tc`, which is what `fgumi dedup` checks on those reads, so it rejects that output too — re-zipper without the flag. Note this release renamed only the tag and the metric: the `fgumi zipper` flag stayed spelled `--skip-pa-tags` through 0.4.0 and was renamed to `--skip-tc-tags` in 0.5.0 ([#617](https://github.com/fulcrumgenomics/fgumi/pull/617)). This entry originally claimed the flag rename landed here; corrected in [#679](https://github.com/fulcrumgenomics/fgumi/issues/679).
