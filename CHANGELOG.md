...
- `fgumi sort --write-index --order coordinate` BAM output bytes now match the corresponding `--write-index`-off run for the same input — multi-threaded BGZF compression is used uniformly. Pre-`#330` Phase 4 the BAM was single-threaded-compressed when `--write-index` was set so the in-sort indexer could track virtual offsets during write; the indexer now runs as a post-write pass via `IndexBamFinalizeHook`, decoupled from BGZF compression. BAI content (record-set returned by samtools region queries) is unchanged; only the BAM's BGZF block layout differs from prior releases.
