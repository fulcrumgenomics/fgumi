...
- `fgumi extract` with `--threads` unset spawns framework workers; the legacy path used 1 inline thread. Overhead is measurable but small.
- `fgumi extract` no longer logs intermediate "Processed N records" lines at 1M-record intervals during single-threaded runs. The typed-step pipeline emits a single "Wrote N records" summary at completion via `ExtractFinalizeHook`.
- `fgumi runall --start-from extract` auto-inserts Correct (when `--correct::umi-files` is set), Align, and Sort as needed based on `--stop-after`. Each insertion is surfaced via a runtime info-log line. See `fgumi runall --help` for details.
...
- The `bwa-mem3` aligner preset no longer requires the `.0123` index file. `bwa-mem3` 0.4.0 pac-fetches the reference from `.pac` on demand and stopped writing `.0123` by default (`fg-labs/bwa-mem3#177`; it is now opt-in via `index --emit-unpacked-ref`, and `mem` ignores any present). fgumi's preset index validation still required `.0123`, so `fgumi runall --start-from align-and-merge --aligner::preset bwa-mem3` (and standalone preset use) failed with "required index file not found: <ref>.0123" against a valid 0.4.0 index. The preset now requires only `.amb`, `.ann`, `.bwt.2bit.64`, and `.pac` — the files both current and older `bwa-mem3` builds produce — so this is backward-compatible with indexes that still carry `.0123`.
