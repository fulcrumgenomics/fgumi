...
/// Runs `fgumi compare metrics` via subprocess and returns `(success, stdout)`.
fn run_compare_subprocess(file1: &Path, file2: &Path, extra_args: &[&str]) -> (bool, String) {
...
fn subprocess_reports_result_differ_and_exit_code_1() {
    ...
    let (success, stdout) = run_compare_subprocess(&f1, &f2, &[]);
    assert!(!success, "differing metrics files must exit non-zero");
...
fn subprocess_reports_result_identical_with_verbose() {
    ...
    let f1 = write_tsv(dir.path(), "a.txt", content);
    let f2 = write_tsv(dir.path(), "b.txt", content);
    let (success, stdout) = run_compare_subprocess(&f1, &f2, &["--verbose"]);
    assert!(success);
...
fn subprocess_quiet_suppresses_output_on_differ() {
    ...
    let f1 = write_tsv(dir.path(), "a.txt", "key\tvalue\nA\t1\n");
    let f2 = write_tsv(dir.path(), "b.txt", "key\tvalue\nA\t2\n");
    let (success, stdout) = run_compare_subprocess(&f1, &f2, &["--quiet"]);
    assert!(!success);
