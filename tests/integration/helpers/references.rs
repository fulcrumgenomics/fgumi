...
/// Check if the named binary is on the PATH.
///
/// Uses `which` (no subprocess invocation) so this is cheap to call in test
/// preludes. Intended as the shared replacement for per-test-file
/// `binary_available` helpers.
...
///
/// Returns `None` so call sites can `return skip_or_fail(...)` from functions
/// that yield an `Option`. Panics loudly when `CI=true` is set in the
/// environment so regressions aren't masked by silent skips on GitHub Actions.
pub fn skip_or_fail(msg: &str) -> Option<PathBuf> {
    assert!(!in_ci(), "CI: {msg}");
    eprintln!("SKIP: {msg}");
    None
}
