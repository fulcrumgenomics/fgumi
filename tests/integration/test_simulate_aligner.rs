...
//!
//! Gated on the `simulate` feature so the binary under test actually has the
//! `simulate aligner` subcommand (matching `test_simulate_sort.rs`); without the
//! gate these tests run against a non-`simulate` build and fail on the
//! feature-not-enabled stub.
