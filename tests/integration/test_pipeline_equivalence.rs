...
// ============================================================================
// Test 9: --start-from zipper equivalence
// ============================================================================

/// Build a queryname-sorted SAM header (SO:queryname, no reference sequences).
fn create_queryname_sorted_header() -> noodles::sam::Header {
    use noodles::sam::header::record::value::Map as HeaderRecordMap;
    ...
    noodles::sam::Header::builder().set_header(header_map).build()
}

/// Build a queryname-sorted SAM header with one reference sequence.
fn create_queryname_sorted_header_with_ref(ref_name: &str, ref_len: usize) -> noodles::sam::Header {
    use bstr::BString;
...
/// Create an unmapped BAM with UMI tags (queryname sorted) for zipper tests.
fn create_unmapped_bam_for_zipper(path: &Path) {
    let header = create_queryname_sorted_header();
    let mut writer =
        bam::io::Writer::new(fs::File::create(path).expect("Failed to create BAM file"));
    writer.write_header(&header).expect("Failed to write header");

    // 5 templates, each with R1 + R2 unmapped reads carrying UMI tags
    for i in 0..5 {
        let name = format!("template_{i}");
        let umi = if i < 3 { "AAAA" } else { "CCCC" };
        // R1
        let r1 = RecordBuilder::new()
            .name(&name)
            .sequence("ACGTACGT")
...
/// Create a mapped SAM (queryname sorted) with aligned reads matching the unmapped
/// BAM read names, for use as zipper input.
fn create_mapped_sam_for_zipper(path: &Path) {
    use noodles::sam;

    let header = create_queryname_sorted_header_with_ref("chr1", 10000);
    ...
    for i in 0..5 {
        ...
        let r2 = RecordBuilder::new()
            ...
            .mate_reverse_complement(false)
            .reference_sequence_id(0)
            .alignment_start(pos + 100)
            .mate_reference_sequence_id(0)
            .mate_alignment_start(pos)
            .template_length(-108)
            .mapping_quality(60)
            .cigar("8M")
            .build();
...
/// Sequential zipper -> sort -> group -> simplex should produce the same consensus
/// as `fgumi pipeline --start-from zipper`.
#[test]
#[allow(clippy::too_many_lines)]
fn test_start_from_zipper_equivalence() {
    let temp_dir = TempDir::new().unwrap();
    let unmapped_bam = temp_dir.path().join("unmapped.bam");
    let mapped_sam = temp_dir.path().join("mapped.sam");
    let ref_path = create_test_reference(temp_dir.path());

    create_unmapped_bam_for_zipper(&unmapped_bam);
    create_mapped_sam_for_zipper(&mapped_sam);

    // --- Sequential path ---
    let zippered_bam = temp_dir.path().join("zippered.bam");
    let sorted_bam = temp_dir.path().join("sorted.bam");
    let grouped_bam = temp_dir.path().join("grouped.bam");
    let sequential_output = temp_dir.path().join("sequential.bam");
    ...
    let seq_count = count_bam_records(&sequential_output);
    let pipeline_count = count_bam_records(&pipeline_output);
    assert!(seq_count > 0, "Sequential zipper path should produce consensus reads");
    assert_eq!(
        seq_count, pipeline_count,
        "Zipper: sequential ({seq_count}) vs pipeline ({pipeline_count}) record count mismatch"
    );
    ...
    assert_eq!(
        seq_seqs, pipeline_seqs,
        "Zipper path consensus sequences should match between sequential and pipeline paths"
    );
}
