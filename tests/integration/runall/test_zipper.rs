//! Byte-comparison: runall `--start-from extract --stop-after zipper` matches
...
/// Run the standalone chain: `fgumi extract` -> `fgumi fastq` -> `bwa mem` ->
/// `fgumi zipper`. Writes the final zippered BAM to `out`.
...
/// Run the pipeline via `fgumi runall --start-from extract --stop-after zipper`.
...
/// Invoke `fgumi compare bams --command zipper`. Returns true if both BAMs are
/// record-equivalent (header differences are ignored).
fn compare_bams(a: &Path, b: &Path) -> bool {
    let output = Command::new(fgumi_bin())
        .args(["compare", "bams", a.to_str().unwrap(), b.to_str().unwrap(), "--command", "zipper"])
        .output()
...
fn test_extract_to_zipper_v2_matches_v1() {
    ...
    assert!(
        ...
        "runall --start-from extract --stop-after zipper output must match \
         v1 extract+fastq+bwa+zipper record-for-record"
    );
}
