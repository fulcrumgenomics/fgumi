...
//! `--consensus simplex` matches the equivalent standalone chain:
...
/// simplex -> filter. Writes the filtered consensus BAM to `out`.
#[allow(
...
fn run_v1_pipeline(r1: &Path, r2: &Path, reference: &Path, out: &Path, tmp_dir: &Path) {
    ...
    let consensus = tmp_dir.join("simplex.bam");
    ...
    let simplex_output = Command::new(fgumi_bin())
        .args([
            "simplex",
            "--input",
            grouped.to_str().unwrap(),
    ...
    assert!(
        simplex_output.status.success(),
        "v1 fgumi simplex failed:\n{}",
        String::from_utf8_lossy(&simplex_output.stderr)
    );
...
#[test]
fn test_extract_to_filter_simplex_v2_matches_v1() {
        ...
        || !crate::helpers::references::tool_on_path("samtools")
    {
        crate::helpers::references::skip_or_fail(
            "bwa/samtools not on PATH; skipping consensus-simplex byte-identity gate",
        );
        return;
    }
    ...
    assert!(
        ...
        "runall --start-from extract --consensus simplex output must match \
         v1 extract+fastq+bwa+zipper+sort+group+simplex+filter record-for-record"
    );
}
