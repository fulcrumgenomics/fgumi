...
//! `fgumi compare bams --command group` so header @PG differences between
...
/// so bwa-mem assigns MAPQ > 0 and reads survive the default
/// `--group::min-mapq=1` filter (which would otherwise drop every record at
/// the group stage and produce a vacuous 0-vs-0 parity comparison).
...
fn test_extract_to_group_v2_matches_v1() {
        ...
        || !crate::helpers::references::tool_on_path("samtools")
    {
        crate::helpers::references::skip_or_fail(
            "bwa/samtools not on PATH; skipping extract-to-group byte-identity gate",
        );
        return;
    }
    let Some(reference) = phix_reference_or_skip("extract-to-group byte-identity gate") else {
        return;
    };
    ...
    // Guard against vacuous parity: with the previous repetitive-ACGT
    // reference, bwa-mem assigned MAPQ=0 to every alignment and the default
    // `--group::min-mapq=1` dropped every record at the group stage, so both
    // pipelines emitted empty BAMs and the comparison was meaningless.
    assert_records_gt_zero(&v1_out, "v1 standalone extract->group chain");
    assert_records_gt_zero(&v2_out, "v2 runall --stop-after group");
