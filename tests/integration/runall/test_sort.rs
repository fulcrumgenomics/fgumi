...
//! missing. Compares via `fgumi compare bams --command sort` so header @PG
...
fn test_extract_to_sort_v2_matches_v1() {
    if !binary_available("bwa") {
        eprintln!("SKIP: bwa not on PATH; skipping extract-to-sort gate test");
        return;
    }
    if !binary_available("samtools") {
        eprintln!("SKIP: samtools not on PATH; skipping extract-to-sort gate test");
        return;
    }
