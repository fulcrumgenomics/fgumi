//! Byte-comparison: runall `--start-from extract --stop-after sort` matches
...
//! `fgumi compare bams --command sort` so header @PG differences between
...
/// so bwa-mem produces realistic alignments with MAPQ > 0 rather than the
/// uniform MAPQ=0 / pos=0 records that the previous repetitive-ACGT
/// reference yielded.
...
fn compare_bams(a: &Path, b: &Path) -> bool {
    let output = Command::new(fgumi_bin())
        .args(["compare", "bams", a.to_str().unwrap(), b.to_str().unwrap(), "--command", "sort"])
        .output()
...
#[test]
fn test_extract_to_sort_v2_matches_v1() {
        ...
        || !crate::helpers::references::tool_on_path("samtools")
    {
        crate::helpers::references::skip_or_fail(
            "bwa/samtools not on PATH; skipping extract-to-sort byte-identity gate",
        );
        return;
    }
    let Some(reference) = phix_reference_or_skip("extract-to-sort byte-identity gate") else {
        return;
    };
    ...
    assert_records_gt_zero(&v1_out, "v1 standalone extract->sort chain");
    assert_records_gt_zero(&v2_out, "v2 runall --stop-after sort");

    assert!(
        compare_bams(&v1_out, &v2_out),
        "runall --start-from extract --stop-after sort output must match \
         v1 extract+fastq+bwa+zipper+sort record-for-record"
    );
}
