...
/// identical per-operation `--metrics` counts, AND an identical record-counter
/// progress summary. The `--metrics` TSV carries only per-operation
/// applied/overwritten/missing counts — not the command's `progress` record
/// counter — so equal TSVs alone do not prove equal progress; this test asserts
/// the `RetagFinalizeHook` "Records processed: N" drain log directly on both
/// paths. This runs the same corpus through both paths — a transcode of one BAM,
/// so the records are identical — and asserts all three, closing the cross-path
/// count gap the BAM-only `cutover_matches_baseline` and the single-op source
/// matrix leave open.
#[test]
fn retag_raw_and_decoded_paths_match() {
    ...
    // Direct record-counter parity: the `--metrics` TSV holds per-operation counts,
    // not the command's `progress` counter, so equal TSVs do not by themselves prove
    // equal progress. Assert the `RetagFinalizeHook` "Records processed: N" summary
    // (its unconditional drain log, read from the shared `progress` counter) on BOTH
    // paths, against the fixture size — no record is filtered, so every input record
    // is processed.
    let expected_progress = format!("Records processed: {}", build_varied_records().len());
    let raw_stderr = String::from_utf8_lossy(&raw.stderr);
    let decoded_stderr = String::from_utf8_lossy(&decoded.stderr);
    assert!(
        raw_stderr.contains(&expected_progress),
        "BAM-source (raw fast path) must report `{expected_progress}`; stderr:\n{raw_stderr}"
    );
    assert!(
        decoded_stderr.contains(&expected_progress),
        "SAM-source (decoded path) must report `{expected_progress}`; stderr:\n{decoded_stderr}"
    );
}
